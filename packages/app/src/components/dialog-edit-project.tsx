  ...
  const saveMutation = useMutation(() => ({
    mutationFn: async () => {
      ...
      if (props.project.id && props.project.id !== "global") {
        ...
        // Reflect the change in the local project list so the left-rail project
        // avatar (which reads `project.icon.color` via `enrich`) updates
        // immediately instead of only after the next backend project refresh.
        ...
        // Keep the per-worktree local override in sync as well. `enrich` reads
        // this `projectMeta` layer to drive the left-rail avatar, so without it
        // an in-app color/icon change would not show until a full reload.
      ...
      // Same local reflection as the non-global branch.
      globalSync.set("project", (list) =>
        list.map((p) =>
          p.id === props.project.id
            ? {
            ...
            <div class="flex gap-3 items-start">
              ...
              >
                <div
                  ...
                  style={
                    !store.iconOverride && !props.project.icon?.url
                      ? { "background-color": getAvatarColors(store.color).background }
                      : undefined
                  }
                  onDrop={handleDrop}
                  ...
                  {(color) => (
                    <button
                      ...
                      classList={{
                        ...
                        "border-2 border-icon-strong-base": store.color === color,
                        "border border-transparent hover:border-border-weak-base":
                          store.color !== color,
                      }}
                      style={{ "background-color": getAvatarColors(color).background }}
                      onClick={() => {
