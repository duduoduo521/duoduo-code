...
export const ModelTooltip: Component<{ model: ModelInfo; latest?: boolean; free?: boolean }> = (props) => {
  ...
  return (
    <div class="flex flex-col gap-1 py-1">
      ...
      <Show when={inputs()}>
        {(value) => (
          <div class="text-12-regular text-text-weak">{language.t("model.tooltip.allows", { inputs: value() })}</div>
        )}
      </Show>
      <div class="text-12-regular text-text-weak">{reasoning()}</div>
      <div class="text-12-regular text-text-weak">{context()}</div>
    </div>
  )
}
