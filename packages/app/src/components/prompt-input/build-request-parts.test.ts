...
describe("buildRequestParts", () => {
    ...
    expect(
      result.requestParts.some(
        (part) =>
          part.type === "text" &&
          part.synthetic &&
          part.metadata?.duoduoComment &&
          (part.metadata.duoduoComment as { comment?: string }).comment === "check this",
      ),
    ).toBe(true)
  ...
  test("handles macOS paths correctly", () => {
    ...
    const result = buildRequestParts({
      ...
      messageID: "msg_mac_1",
      sessionID: "ses_mac_1",
      sessionDirectory: "/Users/kelvin/Projects/duoduo",
    })
    ...
    if (filePart?.type === "file") {
      ...
      expect(() => new URL(filePart.url)).not.toThrow()
      // Should be a normal Unix path
      expect(filePart.url).toBe("file:///Users/kelvin/Projects/duoduo/README.md")
    }
  })
  ...
  // Pasted text is never written to disk, so this is the only path: the content
  // is inlined as a data URL and `source.path` stays empty (nothing to read).
  test("maps pasted text to an inline data URL instead of a temp file", () => {
    const prompt: Prompt = [
      {
        type: "text-file",
        id: "tf_2",
        filename: "pasted-456.txt",
        text: "hello 世界",
        path: "",
      },
    ]
    ...
    const result = buildRequestParts({
      ...
      images: [],
      text: "",
      messageID: "msg_tf_fallback",
      sessionID: "ses_tf_fallback",
      sessionDirectory: "/repo",
    })

    const filePart = result.requestParts.find((part) => part.type === "file" && part.filename === "pasted-456.txt")
    expect(filePart).toBeDefined()
    if (filePart?.type === "file") {
      expect(filePart.url.startsWith("data:text/plain;charset=utf-8;base64,")).toBe(true)
      // Content survives round-trip through base64 (UTF-8 safe)
      const base64 = filePart.url.replace("data:text/plain;charset=utf-8;base64,", "")
      ...
      if (filePart.source?.type === "file") {
        expect(filePart.source.path).toBe("")
        expect(filePart.source.text?.value).toBe("hello 世界")
      }
    }
  })
})
