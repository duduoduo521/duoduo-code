...
export function prependHistoryEntry(
  ...
  const hasTextFiles = prompt.some((part) => part.type === "text-file")
  const hasComments = comments.some((comment) => !!comment.comment.trim())
  if (!text && !hasImages && !hasTextFiles && !hasComments) return entries
...
function isPromptEqual(promptA: PromptHistoryStoredEntry, promptB: PromptHistoryStoredEntry) {
  ...
  for (let i = 0; i < entryA.prompt.length; i++) {
    ...
    if (partA.type === "text-file") {
      if (partA.id !== (partB.type === "text-file" ? partB.id : "")) return false
      if (partA.text !== (partB.type === "text-file" ? partB.text : "")) return false
    }
  }
