...
import { findLast } from "@duoduo-ai/shared/util/array"
import { same } from "@/utils/same"
import { Icon } from "@duoduo-ai/ui/icon"
import { Accordion } from "@duoduo-ai/ui/accordion"
import { StickyAccordionHeader } from "@duoduo-ai/ui/sticky-accordion-header"
...
import type { Message, Part, UserMessage } from "@duoduo-ai/sdk/v2/client"
import { useLanguage } from "@/context/language"
import { useProviders } from "@/hooks/use-providers"
import { useSessionLayout } from "@/pages/session/session-layout"
import { getSessionContextMetrics } from "./session-context-metrics"
import { estimateSessionContextBreakdown, buildBreakdownFromBackend, type SessionContextBreakdownKey } from "./session-context-breakdown"
...
function RawMessageContent(props: { message: Message; getParts: (id: string) => Part[]; onRendered: () => void }) {
  ...
  return (
    <File mode="text" file={file()} class="select-text" onRendered={() => requestAnimationFrame(props.onRendered)} />
  )
}
...
export function SessionContextTab() {
  ...
  const breakdown = createMemo(
    on(
      () => [ctx()?.message.id, ctx()?.contextTokens, ctx()?.breakdown, messages().length, systemPrompt()],
      () => {
        const c = ctx()
        if (!c?.contextTokens) return []
        // Prefer backend-reported breakdown (from Rust TokenBreakdown) when available
        if (c.breakdown) return buildBreakdownFromBackend(c.breakdown, c.contextTokens)
        // Fallback to frontend estimation
  ...
  const breakdownEstimated = createMemo(() => {
    const c = ctx()
    if (!c) return false
    // Backend breakdown explicitly marked estimated, or fallback frontend estimation
    if (c.breakdown) return c.breakdown.estimated === true
    return true
  })
  ...
  const stats = [
    ...
    { label: "context.stats.sessionTotal", value: () => formatter().number(ctx()?.sessionTotal) },
    ...
    { label: "context.stats.outputTokens", value: () => formatter().number(ctx()?.output) },
    {
      ...
      value: () => (ctx()?.reasoning && ctx()!.reasoning! > 0 ? formatter().number(ctx()?.reasoning) : language.t("context.stats.notProvided")),
    },
    {
      label: "context.stats.cacheTokens",
      value: () => {
        const read = ctx()?.cacheRead ?? 0
        const write = ctx()?.cacheWrite ?? 0
        if (read === 0 && write === 0) return language.t("context.stats.notProvided")
        return `${formatter().number(read)} / ${formatter().number(write)}`
      },
    },
    { label: "context.usage.cacheHitRate", value: () => (ctx()?.cacheHitRate != null ? `${ctx()!.cacheHitRate}%` : "—") },
              ...
              <For each={breakdown()}>
                {(segment) => (
                  <div class="flex items-center gap-1 text-11-regular text-text-weak">
                    ...
                    <Show when={breakdownEstimated()}>
                      <div
                        class="rounded px-1 text-10-regular text-text-weaker bg-surface-muted"
                        title={language.t("context.breakdown.estimatedNote")}
                      >
                        {language.t("context.breakdown.estimated")}
                      </div>
                    </Show>
                  </div>
                )}
