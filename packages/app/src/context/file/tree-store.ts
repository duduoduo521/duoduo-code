  ...
  const listDir = (input: string, opts?: { force?: boolean }) => {
    ...
    const promise = options
      ...
      .then((nodes) => {
        ...
        // Merge each node individually with reconcile so that unchanged rows
        // keep a stable reference. The keyed <For> in file-tree.tsx then skips
        // DOM work for those rows, eliminating refresh flicker.
        ...
        if (opts?.force) {
          const refreshDescendants = (parentPath: string) => {
            const childIds = tree.dir[parentPath]?.children
            if (!childIds) return
            for (const childPath of childIds) {
              const childDir = tree.dir[childPath]
              const childNode = tree.node[childPath]
              if (childDir?.expanded && childNode?.type === "directory") {
                void listDir(childPath, { force: true })
              }
            }
          }
          refreshDescendants(dir)
        }
      })
      .catch((e) => {
        if (options.scope() !== directory) return
        const message = errorMessage(e, "Failed to list directory")
        setTree(
          "dir",
          dir,
          produce((draft) => {
