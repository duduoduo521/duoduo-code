...
import type { PermissionRequest } from "@duoduo-ai/sdk/v2/client"
...
export const { use: usePermission, provider: PermissionProvider } = createSimpleContext({
  ...
  init: () => {
    ...
    function shouldAutoRespond(permission: PermissionRequest, directory?: string) {
      const session = directory ? globalSync.child(directory, { bootstrap: false })[0].session : []
      const isAutoAcceptingSession = autoRespondsPermission(store.autoAccept, session, permission, directory)
      if (!isAutoAcceptingSession) return false
      // Auto-accept (session- or directory-level) now blanket-approves ALL
      // permission types, including destructive ones (delete / edit / bash).
      // Enabling the switch is itself the explicit acknowledgment of this risk —
      // the user is shown a risk dialog at the moment they turn it on. The
      // previous "safe-only" restriction has been removed so delete and other
      // dangerous operations are covered by the single auto-accept switch.
      return true
    }
