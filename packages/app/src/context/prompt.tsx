...
/**
 * 由粘贴的大段文本（>= AUTO_ATTACH_CHARS）自动生成的 txt 附件。
 * 字段用 `text`（而非 `content`），使其被全局 content 聚合（blank / 发送主 text / draftText / promptLength）
 * 自然排除，避免把全文重复塞进内联文本部分。内容存于内存，并落盘为临时 txt 文件（path 指向该临时文件，
 * 写入失败则为 ""，发送时 url 降级为 data URL，内容始终经 source.text.value 内联 → 零风险）。
 */
export interface TextFileAttachmentPart {
  type: "text-file"
  id: string
  filename: string
  text: string
...
export type ContentPart = TextPart | FileAttachmentPart | AgentPart | ImageAttachmentPart | TextFileAttachmentPart
...
function isPartEqual(partA: ContentPart, partB: ContentPart) {
  switch (partA.type) {
    ...
    case "text-file":
      return partB.type === "text-file" && partA.id === partB.id && partA.text === partB.text
  }
}
...
function clonePart(part: ContentPart): ContentPart {
  ...
  if (part.type === "text-file") return { ...part }
  return {
    ...part,
