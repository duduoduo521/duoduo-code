...
import { Session, type Message } from "@duoduo-ai/sdk/v2/client"
...
import { showToast, Toast, toaster } from "@duoduo-ai/ui/toast"
...
import { useMemory } from "@/addons/memory/memory-context"
...
export default function Layout(props: ParentProps) {
  ...
  const aim = createAim({
    ...
    onActivate: (directory) => {
      ...
      globalSync.project.loadSessionStatus(directory)
  ...
  function cycleTheme(direction = 1) {
    ...
    showToast({
      ...
      description: themeName(nextThemeId, language.t, theme.name(nextThemeId)),
    })
  }
  ...
  const useUpdatePolling = () =>
    onMount(() => {
      ...
      // Only need updateAndRestart OR (update + restart) for the toast action
      if (!platform.updateAndRestart && !(platform.update && platform.restart)) return
      ...
      const pollUpdate = () =>
        platform.checkUpdate!().then(({ updateAvailable, version }) => {
          ...
          // checkUpdate starts a background download; we watch updateStatus signal
          // to show the toast once download completes, instead of using a fixed timeout.
        })

      // Watch updateStatus to show toast on download completion
      createEffect(() => {
        ...
        toastId = showToast({
          ...
          description: language.t("toast.update.downloaded.description", { version }),
          actions: [
            {
              ...
              onClick: async () => {
                ...
                } else if (platform.update && platform.restart) {
  ...
  const [autoselecting] = createResource(async () => {
    ...
    await ready.promise
    trace("autoselecting: ready.promise resolved")
    await layout.ready.promise
    trace("autoselecting: layout.ready.promise resolved")
    if (!untrack(() => state.autoselect)) return

    trace("autoselecting: before list")
    const list = layout.projects.list()
    const last = server.projects.last()
    trace(`autoselecting: list=${list.length} last=${last}`)
    console.info("[kg] autoselecting on startup", {
  ...
  command.register("layout", () => {
    const commands: CommandOption[] = [
      ...
      {
        id: "about.open",
        title: language.t("command.about.open"),
      ...
      {
        id: "memory.browse",
        title: language.t("command.memory.browse"),
        description: language.t("command.memory.browse.description"),
        ...
        onSelect: () => {
          ...
          if (sessionId && params.dir) {
            ...
            if (!layout.view(key).reviewPanel.opened()) layout.view(key).reviewPanel.open()
            // oxlint-disable-next-line no-floating-promises -- intentional fire-and-forget (UI/SolidJS background work)
            layout.tabs(key).open("memory")
          }
        },
      },
      {
        id: "memory.search",
        title: language.t("command.memory.search"),
        description: language.t("command.memory.search.description"),
        category: language.t("command.category.agent"),
        slash: "memory",
        onSelect: () => {
          const sessionId = params.id
          if (sessionId && params.dir) {
            ...
            // oxlint-disable-next-line no-floating-promises -- intentional fire-and-forget (UI/SolidJS background work)
            layout.tabs(key).open("memory")
            mem.setActiveTab("search")
          }
        },
      },
      {
        id: "memory.stats",
        title: language.t("command.memory.stats"),
        description: language.t("command.memory.stats.description"),
        category: language.t("command.category.agent"),
        slash: "memory",
        onSelect: () => {
          const sessionId = params.id
      ...
      {
        id: "smart-layer.reconnect",
        title: language.t("command.smart-layer.reconnect"),
        description: language.t("command.smart-layer.reconnect.description"),
        category: language.t("command.category.agent"),
        onSelect: () => {
          // oxlint-disable-next-line no-floating-promises -- intentional fire-and-forget (UI/SolidJS background work)
          smartLayer.checkHealth()
          showToast({
            title: language.t("command.smart-layer.reconnect"),
            description: language.t("command.smart-layer.reconnect.description"),
          })
    ...
    for (const [id] of availableThemeEntries()) {
      commands.push({
        id: `theme.set.${id}`,
        title: language.t("command.theme.set", { theme: themeName(id, language.t, theme.name(id)) }),
        category: language.t("command.category.theme"),
  ...
  function connectProvider() {
    ...
    void import("@/components/dialog-custom-provider").then((x) => {
      ...
      dialog.show(() => <x.DialogCustomProvider back="providers" preset="custom" />)
    })
  }
  ...
  function openAbout() {
    const run = ++dialogRun
    void import("@/components/dialog-about").then((x) => {
      if (dialogDead || dialogRun !== run) return
      dialog.show(() => <x.DialogAbout />)
    })
  }

  function openGraph() {
    const project = currentProject()
    const directory = project?.worktree ?? currentDir()
    const run = ++dialogRun
    void import("@/components/dialog-graph").then((x) => {
      if (dialogDead || dialogRun !== run) return
      dialog.show(() => <x.DialogGraph directory={directory} />)
    })
  }
  ...
  async function navigateToProject(directory: string | undefined) {
    ...
    __log(`projectSession=${projectSession?.id ?? "none"}`)
    if (projectSession?.id) {
      await refreshDirs(projectSession.directory)
      __log("refreshDirs done")
      const opened = await openSession(projectSession)
      __log(`openSession(lastProjectSession) opened=${opened}`)
      if (opened) return
    ...
    __log(`latestRootSession(cached)=${latest ? `${latest.directory}/${latest.id}` : "none"}`)
    if (latest && (await openSession(latest))) {
      __log("openSession(latest) opened=true")
      return
    }

    const __tl = performance.now()
    ...
    __log(`session.list x${dirs.length} done in ${(performance.now() - __tl).toFixed(1)}ms`)
    if (fetched && (await openSession(fetched))) {
      __log("openSession(fetched) opened=true")
      return
    }

    __log("fallback navigate to /session")
  ...
  async function openProject(directory: string, shouldNavigate = true) {
    // [TRACE] 临时诊断：定位「打开项目卡死」。定位后删除。
    const __t0 = performance.now()
    const __log = (m: string) =>
      trace(`openProject(${directory}) ${m} (+${(performance.now() - __t0).toFixed(1)}ms)`)
    __log("enter")
    // For saved remote (Plan C) projects, show the opening overlay and probe
    // SSH reachability before mounting the session. A dead server must not
    // mount an empty session — drop the overlay, bounce back to the
    // recent-projects list with a toast. Local projects skip the overlay.
    if (directory.startsWith("remote:")) {
      layout.setOpeningRemote(true)
      // [TRACE] 临时诊断：远程打开兜底超时，避免永久「卡在项目加载中」。定位后删除。
      // 20s 内文件树根未就绪（root.loaded/error）即强制收起遮罩并提示，
      // 区分「后端请求永久 pending」与「前端主线程死循环」两类卡死。
      const guard = setTimeout(() => {
        if (layout.openingRemote()) {
          trace(`openingRemote guard timeout (>20s) for ${directory}`)
    ...
    __log(`checkRemote done ok=${probe.ok}`)
    if (!probe.ok) {
      layout.setOpeningRemote(false)
      showToast({
        variant: "error",
    ...
    __log("projects.open sync part returned")
    if (shouldNavigate) {
      const __r = await navigateToProject(directory)
      __log("navigateToProject resolved")
      return __r
    }
    __log("done without navigate")
  }
  ...
  function closeProject(directory: string, askClear = false) {
    const list = layout.projects.list()
    const key = workspaceKey(directory)
    ...
    // Explicit close: ask whether to keep the index (retention) or wipe it.
    if (askClear) {
      dialog.show(() => (
        <DialogCloseProject
          directory={directory}
          onClosed={() => doCloseProject(directory, key, next)}
        />
      ))
      return
    }
  ...
  // startGraphIndexing — never re-runs, so a project whose index was cleared on
  // close would not auto-index when reopened. Fire-and-forget: dispose also runs
  // the KG disposer (clears in-memory graph, keeps bincode on disk) which is safe
  // here because the index was already handled by closeProjectIndex in the dialog.
  function disposeProjectInstance(directory: string) {
  ...
  function doCloseProject(
    ...
    if (!active) {
      ...
      // stay stranded on the closed project's route. Fallback: when the current
      // route points at the closed project, or no projects remain, navigate away.
      const dirKey = workspaceKey(currentDir() ?? "")
      if (dirKey !== key && layout.projects.list().length > 0) return
      const rest = next ?? layout.projects.list().find((p) => workspaceKey(p.worktree) !== key)
      if (!rest) {
        navigate("/")
        return
      }
      navigateWithSidebarReset(`/${base64Encode(rest.worktree)}/session`)
      queueMicrotask(() => {
        void navigateToProject(rest.worktree)
      })
      return
    }
  ...
  function DialogCloseProject(props: { directory: string; onClosed: () => void }) {
    const [deleteFailed, setDeleteFailed] = createSignal(false)

    // Wipe the index and close the project. Returns true on success, false on failure.
    const clearAndClose = async (): Promise<boolean> => {
      if (!smartLayer.api) {
        console.warn("[kg] clearAndClose: abort", {
          directory: props.directory,
          hasApi: !!smartLayer.api,
        })
        return false
      }
      try {
        console.info("[kg] clearAndClose: wiping index", { directory: props.directory })
        await smartLayer.api.closeProjectIndex(props.directory, true)
        return true
      } catch (e) {
        console.warn("[kg] clearAndClose: failed", { directory: props.directory, error: String(e) })
        return false
      }
    }

    const handleClear = async () => {
      // smartLayer unavailable: cannot wipe, degrade to "keep index" + close
      // so the user is never stuck unable to close.
      if (!smartLayer.api) {
        console.error("[kg] handleClear: service unavailable, keeping index", {
          directory: props.directory,
          hasApi: !!smartLayer.api,
        })
        showToast({
          variant: "error",
          title: language.t("workspace.closeIndex.clearUnavailable"),
        })
        props.onClosed()
        dialog.close()
        return
      }
      const ok = await clearAndClose()
      if (ok) {
        showToast({
          variant: "success",
          title: language.t("settings.graph.clearedOne"),
        })
        props.onClosed()
        dialog.close()
      } else {
        // Deletion failed: do NOT close yet. Surface a choice to the user.
    ...
    const handleKeep = async () => {
      try {
        console.info("[kg] handleKeep: marking index closed (retention)", { directory: props.directory })
        await smartLayer.api?.closeProjectIndex(props.directory, false)
      } catch (e) {
        console.warn("[kg] handleKeep: failed", { directory: props.directory, error: String(e) })
      }
      props.onClosed()
      dialog.close()
    }
    ...
    const handleRetry = async () => {
      ...
      if (ok) {
        ...
        // still failed -> notify and stay on the failure dialog
        showToast({
          variant: "error",
    ...
    return (
      <Dialog title={language.t("workspace.closeIndex.title")} fit>
        <Show
          when={!deleteFailed()}
          fallback={
            <div class="flex flex-col gap-4 px-[var(--dialog-gutter)] pb-5">
              <div class="flex flex-col gap-1">
                <span class="text-14-regular text-text-strong">
                  {language.t("workspace.closeIndex.clearFailed")}
                </span>
                <span class="text-12-regular text-text-weak">
                  {language.t("workspace.closeIndex.clearFailedNote")}
                </span>
              </div>
              <div class="flex justify-end gap-2">
                ...
                <Button variant="primary" size="large" onClick={handleRetry}>
                  {language.t("workspace.closeIndex.retry")}
                </Button>
              </div>
            </div>
          }
        >
          <div class="flex flex-col gap-4 px-[var(--dialog-gutter)] pb-5">
            <div class="flex flex-col gap-1">
              <span class="text-14-regular text-text-strong">
                {language.t("workspace.closeIndex.confirm")}
              </span>
              <span class="text-12-regular text-text-weak">
                {language.t("workspace.closeIndex.note")}
              </span>
            </div>
            <div class="flex justify-end gap-2">
  ...
  // Edit only the SSH credentials (username / password / private key / auth kind) of a
  // remote project. Host/port/remotePath are immutable connection identity and are passed
  // read-only to the form. No project identity (id/worktree) is affected by this action.
  const showEditSshDialog = (project: LocalProject) => {
    if (!project.remote) return
    const run = ++dialogRun
    void import("@/components/dialog-open-project").then((x) => {
      if (dialogDead || dialogRun !== run) return
      dialog.show(() => (
        <x.DialogOpenProject
          onLocal={() => {}}
          ...
          remoteEdit={{
            ...
            port: String(project.remote!.port),
            remotePath: project.remote!.remotePath,
            username: "",
            auth: project.remote!.auth,
          }}
        />
      ))
    })
  ...
  async function chooseProject() {
    ...
    void import("@/components/dialog-open-project").then((x) => {
      ...
      dialog.show(() => (
        <x.DialogOpenProject
          onLocal={openLocalProject}
          onRemoteConnected={(worktree) => {
            dialog.close()
            void openProject(worktree)
  ...
  function DialogDeleteWorkspace(props: { root: string; directory: string }) {
    ...
    return (
      <DialogConfirm
        ...
        danger
        confirmLabel={language.t("workspace.delete.button")}
        message={language.t("workspace.delete.confirm", { name: name() })}
        detail={description()}
        onConfirm={handleDelete}
        onCancel={() => dialog.close()}
      />
  ...
  function DialogResetWorkspace(props: { root: string; directory: string }) {
    ...
    return (
      <DialogConfirm
        ...
        danger
        confirmLabel={language.t("workspace.reset.button")}
        message={language.t("workspace.reset.confirm", { name: name() })}
        detail={`${description()} ${archivedLabel()} ${language.t("workspace.reset.note")}`}
  ...
  const SidebarPanel = (panelProps: {
    ...
    const workspaces = createMemo((prev?: string[]) => {
      const item = project()
      if (!item) return [] as string[]
      const next = workspaceIds(item)
  ...
  const sidebarContent = (mobile?: boolean) => (
    <SidebarContent
      ...
      marketLabel={() => language.t("gearStore.title")}
      onOpenMarket={openMarket}
      graphLabel={() => language.t("graphKanban.title")}
