...
import { HoverCard } from "@duoduo-ai/ui/hover-card"
  ...
  return (
    ...
    >
      <ContextMenu.Trigger
        ...
        classList={{
          "flex items-center justify-center size-10 rounded-lg overflow-hidden transition-colors cursor-default": true,
      ...
      <ContextMenu.Portal>
        <ContextMenu.Content>
          ...
          <Show when={props.project.remote}>
            <ContextMenu.Item onSelect={() => props.showEditSshDialog(props.project)}>
              <ContextMenu.ItemLabel>{props.language.t("edit_ssh_info")}</ContextMenu.ItemLabel>
            </ContextMenu.Item>
          </Show>
          <ContextMenu.Item
...
export const SortableProject = (props: {
  ...
  const sortable = createSafeSortable(props.project.worktree)
  const selected = createMemo(
    () => workspaceKey(props.ctx.currentProject()?.worktree ?? "") === workspaceKey(props.project.worktree),
  )
  ...
  const tile = () => (
    <ProjectTile
      ...
      showEditSshDialog={props.ctx.showEditSshDialog}
