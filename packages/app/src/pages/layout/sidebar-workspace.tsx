...
export const SortableWorkspace = (props: {
  ...
  const globalSync = useGlobalSync()
  const language = useLanguage()
  const sortable = createSafeSortable(props.directory)
  ...
  const hasMore = createMemo(() => !workspaceStore.exhausted)
  ...
  const loadMore = async () => {
    setWorkspaceStore("limit", (limit) => (limit ?? 0) + 100)
...
export const LocalWorkspace = (props: {
  ...
  const count = createMemo(() => sessions()?.length ?? 0)
  const query = useQuery(() => ({ ...loadSessionsQuery(props.project.worktree) }))
  const hasMore = createMemo(() => !workspace().store.exhausted)
  const loading = () => query.isLoading && count() === 0
  const loadMore = async () => {
    workspace().setStore("limit", (limit) => (limit ?? 0) + 100)
