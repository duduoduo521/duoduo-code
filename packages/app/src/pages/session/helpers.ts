...
export const createSessionTabs = (input: TabsInput) => {
  ...
  const closableTab = createMemo(() => {
    const active = activeTab()
    if (active === "context" || active === "memory") return active
    if (!openedTabs().includes(active)) return
    return active
  })
...
export const createOpenReviewFile = (input: {
  ...
  loadFile: (path: string, options?: { force?: boolean }) => any
}) => {
  return (path: string) => {
    batch(() => {
      const maybePromise = input.loadFile(path, { force: true })
      const open = () => {
...
export const createOpenSessionFileTab = (input: {
  ...
  openTab: (tab: string) => void
  pathFromTab: (tab: string) => string | undefined
  loadFile: (path: string, options?: { force?: boolean }) => void
  ...
  return (value: string) => {
    ...
    // Always force reload when opening a file from the file tree to ensure
    // the latest content is displayed, even if a watcher event was missed.
    input.loadFile(path, { force: true })
