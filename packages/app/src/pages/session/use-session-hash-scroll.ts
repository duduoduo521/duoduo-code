import type { UserMessage } from "@duoduo-ai/sdk/v2"
...
export const useSessionHashScroll = (input: {
  ...
  const scrollToMessage = (message: UserMessage, behavior: ScrollBehavior = "smooth") => {
    ...
    if (index !== -1) {
      const start = input.turnStart()
      const end = input.turnEnd()
      const total = visibleUserMessages().length
      // Target outside the rendered window (bounded by turnStart/turnEnd): expand
      // the window to include it before seeking so the element exists in the DOM.
      if (index < start || index >= end) {
        input.setTurnStart(Math.min(start, index))
        input.setTurnEnd(Math.min(total, Math.max(end, index + 1)))
