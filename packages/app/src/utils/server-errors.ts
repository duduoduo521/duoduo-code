...
export function formatServerError(error: unknown, translate?: Translator, fallback?: string) {
  ...
  // Plain JS Error — surface the real message.
  if (error instanceof Error && error.message) return error.message

  // Plain string.
  if (typeof error === "string" && error) return error

  // Structured objects: surface real info instead of a generic "Unknown error".
  if (typeof error === "object" && error !== null) {
    ...
    // Common API error envelope: { data: { message } }.
    if (typeof o.data === "object" && o.data !== null) {
      const dataMessage = (o.data as Record<string, unknown>).message
      if (typeof dataMessage === "string" && dataMessage) return dataMessage
    }

    // InitError-like: { name, data } → name + JSON of data.
    if (typeof o.name === "string") {
      const data = o.data
      if (typeof data === "object" && data !== null) {
        const json = safeJson(data)
        if (json && json !== "{}") return `${o.name}: ${json}`
      }
      return o.name
    }

    // Anything else: best-effort JSON.
    const json = safeJson(error)
    if (json && json !== "{}" && json !== "null") return json
  }

  // Numbers/booleans/null/undefined without a fallback.
...
function parseReadableProviderModelNotFoundError(errorInput: ProviderModelNotFoundError, translator?: Translator) {
  ...
  const tail = tr(translator, "error.chain.checkConfig", "Check your config (duoduo-ai.json) provider/model names")
  if (list.length) {
