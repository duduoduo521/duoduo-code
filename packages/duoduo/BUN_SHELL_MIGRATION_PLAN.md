...
Replace runtime Bun shell template-tag usage in `packages/duoduo/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/duoduo/src` is removed except:
