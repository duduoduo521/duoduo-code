...
Practical notes for an eventual migration of `packages/duoduo` server routes from the current Hono handlers to Effect `HttpApi`, either as a full replacement or as a parallel surface.
...
The bridge is gated behind `DUODUOCODE_EXPERIMENTAL_HTTPAPI` (or `DUODUOCODE_EXPERIMENTAL`). When the flag is off (default), all requests go through the original Hono handlers unchanged.

```ts
// in instance/index.ts
if (Flag.DUODUOCODE_EXPERIMENTAL_HTTPAPI) {
...
6. Mount the new surface in parallel behind the `DUODUOCODE_EXPERIMENTAL_HTTPAPI` bridge.
...
- routes keep their normal instance paths and are gated by the `DUODUOCODE_EXPERIMENTAL_HTTPAPI` flag
...
- the bridged `HttpApi` layer resolves instance context via an `HttpRouter.middleware` that reads `x-duoduo-directory` headers and `directory` query params
