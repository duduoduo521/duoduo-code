...
Practical reference for migrating data types in `packages/duoduo` from
...
All of `packages/duoduo/src/config/` has been migrated. Files that still
...
> **Layer A (strategy B2):** the **published** LLM contract for the
> dispatch-only tools `glob` / `write` / `task` / `proceed_to_*` / `apply_patch`
> lives in Rust (`crates/agent-executor/src/agentic_loop.rs` via `xxx_tool()`
> factories). The zod schemas in `src/tool/*.ts` are the type-safe mirror used
> only for `execute`. When migrating these tools to Effect Schema, keep the Rust
> `description` / `required` text verbatim in sync — do **not** make TS the
> contract source. See `tools.md` → "Layer A: Rust tool schema single-source".
