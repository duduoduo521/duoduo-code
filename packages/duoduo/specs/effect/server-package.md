...
Practical reference for extracting a future `packages/server` from the current `packages/duoduo` monolith while `packages/core` is still being migrated to Effect.
...
- `packages/core` - all duoduo services, Effect-first source of truth
- `packages/server` - duoduo server, with separate contract and implementation, still producing `openapi.json`
...
- import from `server` and embed the full duoduo server into an existing Node app
- spawn the CLI and talk to the server through that boundary

## Current state

Everything still lives in `packages/duoduo`.
...
- those slices are mounted into the Hono server behind `DUODUOCODE_EXPERIMENTAL_HTTPAPI`
...
- the current server host still lives in `packages/duoduo`
...
- if `packages/server` imports services from `packages/duoduo` while `packages/duoduo` imports `packages/server` to host routes, we create a package cycle immediately
...
4. keep `packages/duoduo` as the temporary Hono host
...
- `packages/server` must not import from `packages/duoduo`

Allowed in phase 1:

- `packages/duoduo` imports `packages/server`
...
- `packages/duoduo` shrinks or disappears as package responsibilities are fully split
...
- `packages/duoduo` keeps generating legacy OpenAPI from Hono routes
...
- expose host adapters needed by `packages/duoduo`
...
Keep in `packages/duoduo` for now:
...
- host mounting from `packages/duoduo`
...
- `packages/server` must still not import from `packages/duoduo`
...
### PR 4. Mount `packages/server` question from `packages/duoduo`

Scope:

- replace local experimental question route wiring in `packages/duoduo`
...
- mount it in parallel from `packages/duoduo`
...
- move adapter selection and server startup out of `packages/duoduo`

Rules:

- do not start this while `packages/server` still depends on `packages/duoduo`
...
- host logic no longer lives in `packages/duoduo`
...
3. the route is mounted from the current host in `packages/duoduo`
...
- `packages/server` imports services or runtime from `packages/duoduo`
- `packages/duoduo` imports route definitions or handlers from `packages/server`

Avoid by:

- keeping phase-1 `packages/server` free of `packages/duoduo` imports
...
- [ ] mount `question` from `packages/duoduo`
...
2. keep `packages/duoduo` as the temporary host
...
If a proposed PR would make `packages/server` import from `packages/duoduo`, stop and restructure the boundary first.
