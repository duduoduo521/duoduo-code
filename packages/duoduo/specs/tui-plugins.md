...
- Author package entrypoint is `@duoduo-ai/plugin/tui`.
...
```json
{
  ...
  "theme": "smoke-theme",
  "plugin": ["@acme/duoduo-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  }
}
...
- Import types from `@duoduo-ai/plugin/tui`.
- `@duoduo-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@duoduo-ai/plugin/tui"
...
- The value is a semver range checked against the running DuoDuoCode version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.duoduo` is absent, no check is performed (backward compatible).
...
- `duoduocode plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `duoduocode plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.duoduo` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.duoduo`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.duoduo`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.duoduo` dir or `DUODUO_CONFIG_DIR`, DuoDuoCode installs `@duoduo-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@duoduo-ai/plugin/tui`.
...
- Local plugins persist installed themes under the local `.duoduo/themes` area near the plugin config source.
...
- `--pure` / `DUODUO_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.duoduo/plugins/tui-smoke.tsx`
- Local vim plugin: `.duoduo/plugins/tui-vim.tsx`
- Local smoke config: `.duoduo/tui.json`
- Local smoke theme: `.duoduo/plugins/smoke-theme.json`
