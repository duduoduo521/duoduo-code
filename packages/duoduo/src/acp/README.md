...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for duoduo.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal duoduo sessions
...
duoduocode acp --cwd /path/to/project
```
...
```bash
DUODUO_ENABLE_QUESTION_TOOL=1 duoduocode acp
```
...
- **Terminal Integration**: Full terminal support via duoduo's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | duoduocode acp
```
...
### Mapping to DuoDuoCode

ACP sessions map cleanly to duoduo's internal session model:
