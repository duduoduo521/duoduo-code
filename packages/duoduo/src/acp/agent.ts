...
export async function init({ sdk: _sdk }: { sdk: DuoDuoClient }) {
  return {
...
export class Agent implements ACPAgent {
      ...
      case "message.part.updated": {
        ...
        if (part.type === "tool") {
          ...
          switch (part.state.status) {
            ...
            case "completed": {
              await this.sendToolCallCompleted(sessionId, part)
              return
            }
            case "error":
              await this.sendToolCallError(sessionId, part)
              return
          }
        }
  ...
  async initialize(params: InitializeRequest): Promise<InitializeResponse> {
    ...
    const authMethod: AuthMethod = {
      description: "Run `duoduo auth login` in the terminal",
      name: "Login with duoduo",
      id: "duoduo-login",
    }
  ...
  private async processMessage(message: SessionMessageResponse) {
    ...
    for (const part of message.parts) {
      if (part.type === "tool") {
        ...
        // DuoDuoCode stores files internally as { type: "file", url, filename, mime }.
  ...
  /**
   * Build the content array for a completed tool call, including diff for edit tools.
   * Shared by handleEvent() and processMessage().
   * Caller must ensure part.state.status === "completed".
   */
  private buildCompletedContent(part: ToolPart, kind: ToolKind): ToolCallContent[] {
  ...
  /**
   * Send a completed tool_call_update to the ACP connection.
   * Handles todo parsing for todowrite tools internally.
   * Shared by handleEvent() and processMessage().
   * Caller must ensure part.state.status === "completed".
   */
  private async sendToolCallCompleted(sessionId: string, part: ToolPart): Promise<void> {
    this.toolStarts.delete(part.callID)
    this.bashSnapshots.delete(part.callID)
    const state = part.state as ToolStateCompleted
    const kind = toToolKind(part.tool)
    const content = this.buildCompletedContent(part, kind)

    if (part.tool === "todowrite") {
      const parsedTodos = z.array(Todo.Info).safeParse(JSON.parse(state.output))
  ...
  /**
   * Send a failed tool_call_update to the ACP connection.
   * Shared by handleEvent() and processMessage().
   * Caller must ensure part.state.status === "error".
   */
  private async sendToolCallError(sessionId: string, part: ToolPart): Promise<void> {
    this.toolStarts.delete(part.callID)
    this.bashSnapshots.delete(part.callID)
...
async function defaultModel(config: ACPConfig, cwd?: string): Promise<{ providerID: ProviderID; modelID: ModelID }> {
  ...
  throw new Error("no models available - please add a provider in settings")
}
