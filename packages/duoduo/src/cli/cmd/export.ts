...
export const ExportCommand = cmd({
  ...
  handler: async (args) => {
    await bootstrap(process.cwd(), async () => {
      ...
      // Only emit progress on a real terminal; in pipe/CI mode this would
      // pollute stderr and break automated consumers.
      ...
      if (!sessionID) {
        ...
        const sessions = []
        // Export is a backup/migration operation: it must capture ALL sessions,
        // not just the most recent 100. Session.list() defaults to limit=100,
        // so we explicitly request the full range.
        for await (const session of Session.list({ limit: Number.MAX_SAFE_INTEGER })) {
