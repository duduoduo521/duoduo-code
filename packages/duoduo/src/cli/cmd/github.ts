...
const AGENT_USERNAME = "duoduo-agent[bot]"
const AGENT_REACTION = "eyes"
const WORKFLOW_FILE = ".github/workflows/duoduo.yml"
      ...
      async fn() {
        {
          ...
          function printNextSteps() {
            ...
            prompts.outro(
              [
                ...
                "",
                `   Learn more about the GitHub agent - ${APP_DOCS}/github/#usage-examples`,
              ].join("\n"),
            )
          }
          ...
          async function installGitHubApp() {
            prompts.log.warn(
              "GitHub App installation requires the Duoduo GitHub App backend, which is not available in this build. Please install the app manually at https://github.com/apps/duoduo-agent",
            )
          }
...
jobs:
  duoduo:
    if: |
      ...
      contains(github.event.comment.body, ' /duoduo') ||
      startsWith(github.event.comment.body, '/duoduo')
    ...
    steps:
      ...
      - name: Run duoduo
        uses: duduoduo521/duoduocode/github@latest
        with:
    ...
    await bootstrap(process.cwd(), async () => {
      ...
      try {
        if (useGithubToken) {
          ...
          throw new Error(
            "OIDC-based GitHub App token exchange requires the Duoduo GitHub App backend, which is not available in this build. Use use_github_token with GITHUB_TOKEN instead.",
          )
        }
      ...
      async function getUserPrompt() {
        ...
        const mentions = (process.env["MENTIONS"] || "/duoduo,/oc")
          .split(",")
      ...
      async function subscribeSessionEvents() {
        ...
        let text = ""
        await Bus.subscribe(MessageV2.Event.PartUpdated, (evt) => {
      ...
      async function chat(message: string, files: PromptFiles = []) {
        console.log("Sending message to duoduo...")
        ...
        return AppRuntime.runPromise(
          Effect.gen(function* () {
            ...
            if (result.info.role === "assistant" && result.info.error) {
              ...
              if (MessageV2.ContextOverflowError.isInstance(err)) throw new Error(formatPromptTooLargeError(files))
      ...
      async function getOidcToken() {
        try {
          return await core.getIDToken("duoduo-github-action")
        } catch (error) {
      ...
      function generateBranchName(type: "issue" | "pr" | "schedule" | "dispatch") {
        ...
        if (type === "schedule" || type === "dispatch") {
          ...
          return `duoduo/${type}-${hex}-${timestamp}`
        }
        return `duoduo/${type}${issueId}-${timestamp}`
      }
  ...
  repository(owner: $owner, name: $repo) {
    issue(number: $number) {
      ...
      comments(first: 100) {
        ...
        if (!issueResult.repository) throw new Error(`Repository ${owner}/${repo} not found`)
      ...
      function buildPromptDataForIssue(issue: GitHubIssue) {
        ...
        return [
          ...
          "- Git push and PR creation are handled AUTOMATICALLY by the duoduo infrastructure after your response",
  ...
  repository(owner: $owner, name: $repo) {
    pullRequest(number: $number) {
      ...
      reviews(first: 100) {
        ...
        if (!prResult.repository) throw new Error(`Repository ${owner}/${repo} not found`)
