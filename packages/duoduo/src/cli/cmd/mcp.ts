    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (servers.length === 0) {
          ...
          prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in duoduo-ai.json:")
          prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
    ...
    "${serverName}": {
      ...
      "oauth": {
        ...
        "clientSecret": "your-client-secret"
            ...
            spinner.stop("Authentication requires manual action")
            prompts.log.warn("Could not open browser automatically.")
            prompts.log.info("Open the URL above in your browser, then re-run this command to complete authentication.")
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .duoduo/ subdirectory too)
  const candidates = [path.join(baseDir, "duoduo-ai.json"), path.join(baseDir, "duoduo-ai.jsonc")]

  if (!global) {
    const pd = projectDataDir(baseDir)
    candidates.push(path.join(pd, "duoduo-ai.json"), path.join(pd, "duoduo-ai.jsonc"))
  }
  ...
  // Default to duoduo-ai.json if none exist
  return candidates[0]
}
      ...
      async fn() {
        ...
        if (type === "local") {
          const command = await prompts.text({
            ...
            placeholder: "e.g., duoduocode x @modelcontextprotocol/server-filesystem",
            validate: (x) => (x && x.length > 0 ? undefined : "Required"),
          })
          ...
          const response = await fetch(serverConfig.url, {
            ...
            body: JSON.stringify({
              ...
              params: {
                ...
                capabilities: {},
                clientInfo: { name: "duoduo-debug", version: InstallationVersion },
              },
              id: 1,
            }),
          })
