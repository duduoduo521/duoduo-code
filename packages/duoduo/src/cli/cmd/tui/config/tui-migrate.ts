...
import { APP_TUI_SCHEMA } from "@/config/domains"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from duoduo-ai.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const duoduoConfigs = await duoduoFiles(input)
  for (const file of duoduoConfigs) {
...
async function duoduoFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "duoduo"),
    ...(await Filesystem.findUp(["duoduo-ai.json", "duoduo-ai.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "duoduo"))
  }
  if (Flag.DUODUO_CONFIG) files.push(Flag.DUODUO_CONFIG)
