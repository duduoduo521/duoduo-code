...
import duoduoTheme from "./theme/duoduo-ai.json" with { type: "json" }
...
import type { TuiThemeCurrent } from "@duoduo-ai/plugin/tui"
  ...
  init: (props: { mode: "dark" | "light" }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "duoduo")
        draft.active = typeof active === "string" ? active : "duoduo"
        draft.ready = false
      }),
    )
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes["duoduo"]!, store.mode)
    })
