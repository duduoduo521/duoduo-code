...
import type { InternalTuiPlugin } from "./internal"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir = path.join(projectDataDir(source_dir), "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(projectDataDir(state.directory), "tui.json"),
  }
}
...
async function installPluginBySpec(
  ...
  raw: string,
  global = false,
  opts: { allowScripts?: boolean; integrity?: string } = {},
  ...
  const install = await installModulePlugin(spec, {
    ...
    expectedIntegrity: opts.integrity,
  })
  if (!install.ok) {
...
export async function installPlugin(
  spec: string,
  options?: { global?: boolean; allowScripts?: boolean; integrity?: string },
) {
  return installPluginBySpec(runtime, spec, options?.global, {
    allowScripts: options?.allowScripts,
    integrity: options?.integrity,
...
async function load(input: { api: Api; config: TuiConfig.Info }) {
  ...
  try {
    await Instance.provide({
      directory: cwd,
      fn: async () => {
        const records = Flag.DUODUO_PURE ? [] : (config.plugin_origins ?? [])
        if (Flag.DUODUO_PURE && config.plugin_origins?.length) {
          log.info("skipping external tui plugins in pure mode", { count: config.plugin_origins.length })
        }

        if (!Flag.DUODUO_PURE) {
          const { INTERNAL_TUI_PLUGINS } = await import("./internal")
