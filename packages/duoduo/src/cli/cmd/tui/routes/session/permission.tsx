          ...
          body={
            <Switch>
              <Match when={props.request.always.length === 1 && props.request.always[0] === "*"}>
                <TextBody title={"This will allow " + props.request.permission + " until DuoDuoCode is restarted."} />
              </Match>
              <Match when={true}>
                <box paddingLeft={1} gap={1}>
                  <text fg={theme.textMuted}>This will allow the following patterns until DuoDuoCode is restarted</text>
                  <box>
  ...
  return (
    ...
    >
      <box gap={1} paddingLeft={1} paddingRight={3} paddingTop={1} paddingBottom={1}>
        ...
        <box paddingLeft={1}>
          <text fg={theme.textMuted}>Tell DuoDuoCode what to do differently</text>
        </box>
      </box>
      <box
...
function Prompt<const T extends Record<string, string>>(props: {
  ...
  const diffKey = Keybind.parse("ctrl+f")[0] ?? null
  const narrow = createMemo(() => dimensions().width < 80)
  const dialog = useDialog()
