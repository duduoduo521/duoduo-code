...
import { APP_CONFIG_SCHEMA, APP_DOCS } from "./domains"
...
function normalizeLoadedConfig(data: unknown, source: string) {
  ...
  if (hadLegacy) {
    ...
    delete copy.tui
    log.warn("tui keys in duoduo config are deprecated; move them to tui.json", { path: source })
  }
  // Migrate: provider model `temperature` used to be a boolean capability flag
  // and is now a numeric sampling value. Any legacy boolean is moved into
  // `supports_temperature` so the new numeric schema parses cleanly.
    ...
    for (const p of Object.values(provider)) {
      ...
      for (const m of Object.values(models)) {
        ...
        if (typeof m.temperature === "boolean") {
          m.supports_temperature = m.temperature
...
export const Info = Schema.Struct({
  ...
  server: Schema.optional(ConfigServer.Server).annotate({
    description: "Server configuration for duoduo serve and web commands",
  }),
  command: Schema.optional(Schema.Record(Schema.String, ConfigCommand.Info)).annotate({
    description: `Command configuration, see ${APP_DOCS}/commands`,
  }),
  ...
  watcher: Schema.optional(
    Schema.Struct({
      ...
      pollInterval: Schema.optional(PositiveInt).annotate({
        description: "Milliseconds between scans when 'poll' is enabled. Defaults to 4000.",
      }),
    }),
  ),
  ...
  snapshot_retention_days: Schema.optional(PositiveInt).annotate({
    description:
  ...
  lan_scan_ports: Schema.optional(Schema.Array(Schema.Number)).annotate({
    description: "Custom LAN scan ports in addition to default ports (11434, 1234, 8080, 8000, 23333, 30000)",
  }),
  ...
  ).annotate({ description: `Agent configuration, see ${APP_DOCS}/agents` }),
  ...
  quality: Schema.optional(
    Schema.Struct({
      enableLlmCheck: Schema.optional(Schema.Boolean).annotate({
        description:
  ...
  ).annotate({ description: "Quality-gate settings for cascade edits." }),
})
...
export class Service extends Context.Service<Service, Interface>()("@duoduocode/Config") {}

function globalConfigFile() {
  const candidates = ["duoduo-ai.jsonc", "duoduo-ai.json", "config.json"].map((file) =>
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      // `$schema` is only a hint for editors, so default it in memory rather than
      // rewriting the file. That file may be the user's own — or, for a Plan C
      // mirror, content synced from the server — and any edit written here would
      // be uploaded to the remote on the next push.
      if (!data.$schema) data.$schema = APP_CONFIG_SCHEMA
      return data
    })
    ...
    const loadGlobal = Effect.fnUntraced(function* () {
      let result: Info = pipe(
        ...
        mergeDeep(yield* loadFile(path.join(Global.Path.config, "duoduo-ai.json"))),
        mergeDeep(yield* loadFile(path.join(Global.Path.config, "duoduo-ai.jsonc"))),
      )
        ...
        yield* Effect.promise(() =>
          import(pathToFileURL(legacy).href, { with: { type: "toml" } })
            .then(async (mod) => {
              ...
              result["$schema"] = APP_CONFIG_SCHEMA
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "DUODUO_CONFIG_CONTENT") return "local"
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            log.debug("fetching remote config", { url: `${url}/.well-known/duoduo` })
            const response = yield* Effect.promise(() => fetch(`${url}/.well-known/duoduo`))
            if (!response.ok) {
              throw new Error(`failed to fetch remote config from ${url}: ${response.status}`)
            }
            const wellknown = (yield* Effect.promise(() => response.json())) as { config?: Record<string, unknown> }
            const remoteConfig = wellknown.config ?? {}
            if (!remoteConfig.$schema) remoteConfig.$schema = APP_CONFIG_SCHEMA
            const source = `${url}/.well-known/duoduo`
// @effect-diagnostics-next-line preferSchemaOverJson:off
        ...
        if (Flag.DUODUO_CONFIG) {
          yield* merge(Flag.DUODUO_CONFIG, yield* loadFile(Flag.DUODUO_CONFIG))
          log.debug("loaded custom config", { path: Flag.DUODUO_CONFIG })
        }

        if (!Flag.DUODUO_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("duoduo", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        if (Flag.DUODUO_CONFIG_DIR) {
          log.debug("loading config from DUODUO_CONFIG_DIR", { path: Flag.DUODUO_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void, never>[] = []

        for (const dir of directories) {
          if (dir === projectDataDir(ctx.directory) || dir === Flag.DUODUO_CONFIG_DIR) {
            const configFiles = ["duoduo-ai.json", "duoduo-ai.jsonc", "duoduo.json", "duoduo.jsonc"]
          ...
          // Only directories we own get a generated `.gitignore`. A `.duoduo`
          // folder found *inside* the project belongs to the user (and for a
          // Plan C mirror it is synced content), so writing into it would be
          // uploaded to the remote on the next push.
          const insideProject = [ctx.directory, ctx.worktree].some(
            (root) => !!root && dir.startsWith(root),
          )
          if (!insideProject) yield* ensureGitignore(dir).pipe(Effect.orDie)

          // Background dependency install: detached on purpose.
          // forkScoped would tie the installer fiber to the Config entry scope,
          // and disposeInstance -> ScopedCache.invalidate -> Scope.close would
          // then synchronously interrupt it (waiting on the shared flock and
          // the Arborist run), stalling project teardown for seconds. Unlike
          // the knowledge-graph pass (forkDetach + registerInstanceCanceller
          // because it must be stopped), this install is a one-shot
          // maintenance task: it terminates on its own, failures only warn
          // below, and npm re-runs idempotently on the next attempt.
          const dep = yield* npmSvc
            .install(dir, {
              add: [
                {
                  name: "@duoduo-ai/plugin",
                  version:
          ...
          // Auto-discovered plugins under `.duoduo/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.DUODUO_CONFIG_CONTENT) {
          const source = "DUODUO_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.DUODUO_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          log.debug("loaded custom config from DUODUO_CONFIG_CONTENT")
        }
        ...
        if (existsSync(managedDir)) {
          for (const file of ["duoduo-ai.json", "duoduo-ai.jsonc", "duoduo.json", "duoduo.jsonc"]) {
        ...
        if (Flag.DUODUO_PERMISSION) {
// @effect-diagnostics-next-line preferSchemaOverJson:off
          result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.DUODUO_PERMISSION))
        }
        ...
        if (Flag.DUODUO_DISABLE_AUTOCOMPACT) {
    ...
    const update = Effect.fn("Config.update")(function* (config: Info) {
      ...
      // Per-project config is persisted in the data directory, never in the
      // project tree: the tree holds user content, and for a Plan C remote
      // mirror anything written there gets uploaded on the next push.
      // The filename must match what the read loop loads (`duoduo-ai.json`,
      // see the `configFiles` list below) — otherwise a programmatic update
      // would be written to `config.json` and never read back.
      const file = path.join(projectDataDir(dir), "duoduo-ai.json")
      const existing = yield* loadFile(file)
      const merged = mergeDeep(writable(existing), writable(config))
      // Only write if config actually changed to avoid Tauri restart loop in dev mode
// @effect-diagnostics-next-line preferSchemaOverJson:off
