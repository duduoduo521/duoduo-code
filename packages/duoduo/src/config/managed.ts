...
const MANAGED_PLIST_DOMAIN = "ai.duoduocode.managed"

// Keys injected by macOS/MDM into the managed plist that are not DuoDuo config
...
function systemManagedConfigDir(): string {
  switch (process.platform) {
    case "darwin":
      return "/Library/Application Support/duoduo"
    case "win32":
      return path.join(process.env.ProgramData || "C:\\ProgramData", "duoduocode")
    default:
      return "/etc/duoduo"
  }
}
...
export function managedConfigDir() {
  return process.env.DUODUO_TEST_MANAGED_CONFIG_DIR || systemManagedConfigDir()
}
