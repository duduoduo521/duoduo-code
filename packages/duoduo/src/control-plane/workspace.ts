...
export const create = fn(CreateInput, async (input) => {
  ...
  const env = {
    DUODUO_WORKSPACE_ID: config.id,
    DUODUO_EXPERIMENTAL_WORKSPACES: "true",
  ...
  // Fire-and-forget: sync runs in the background while we await the status
  // event below. Catch rejections so they don't become unhandled.
  void startSync(info).catch((err) => Log.Default.error("startSync failed", { err }))
...
export const remove = fn(WorkspaceID.zod, async (id) => {
  const sessions = Database.useProject((db) =>
...
async function syncHistory(space: Info, url: URL | string, headers: HeadersInit | undefined, signal: AbortSignal) {
  const sessionIDs = Database.useProject((db) =>
    db
...
async function startSync(space: Info) {
  if (!Flag.DUODUO_EXPERIMENTAL_WORKSPACES) return
...
export function startWorkspaceSyncing(projectID: ProjectID) {
  // Cross-DB JOIN split: query WorkspaceTable (global DB) and SessionTable
  // (project DB) separately, then intersect.
  const workspaceIDs = new Set(
    Database.useProject((db) => db.select({ id: SessionTable.workspace_id }).from(SessionTable).all())
      .map((row) => row.id)
  ...
  const spaces = Database.use((db) =>
    db.select({ workspace: WorkspaceTable }).from(WorkspaceTable).where(eq(WorkspaceTable.project_id, projectID)).all(),
  )

  for (const row of spaces.filter((row) => workspaceIDs.has(row.workspace.id))) {
    void startSync(fromRow(row.workspace))
  }
}
