...
export const make = <A, E = never>(
  ...
  const ensureRunning = (work: Effect.Effect<A, E>) =>
    SynchronizedRef.modifyEffect(
      ...
      Effect.catchCause((cause): Effect.Effect<A, E> => {
        if (Cause.hasInterruptsOnly(cause) && onInterrupt) return onInterrupt
    ...
    switch (st._tag) {
      ...
      case "ShellThenRun":
        return [
          Effect.gen(function* () {
            if (onInterrupt) {
              const interrupted = yield* onInterrupt.pipe(Effect.exit)
              yield* Deferred.done(st.run.done, interrupted).pipe(Effect.asVoid)
            } else {
