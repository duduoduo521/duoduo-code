...
import { sanitizedProcessEnv } from "@/util/duoduo-process"
...
export class Service extends Context.Service<Service, Interface>()("@duoduocode/Ripgrep") {}
...
function aborted(signal?: AbortSignal) {
  ...
  const out = new DuoduoError({ message: "Aborted", messageZh: "已中止", cause: undefined })
  out.name = "AbortError"
  return out
}
...
function error(stderr: string, code: number) {
  const err = new DuoduoError({ message: String(stderr.trim() || `ripgrep failed with code ${code}`), messageZh: String(stderr.trim() || `ripgrep 执行失败，退出码 ${code}`), cause: undefined })
...
function parse(line: string) {
  return Effect.try({
    ...
    catch: (cause) => new DuoduoError({ message: "invalid ripgrep output", messageZh: "ripgrep 输出无效", cause: cause }),
  })
}
...
// @effect-diagnostics-next-line unnecessaryFailYieldableError:off
          return yield* Effect.fail(new DuoduoError({ message: String(`ripgrep archive did not contain executable: ${extracted}`), messageZh: String(`ripgrep 压缩包中未包含可执行文件：${extracted}`), cause: undefined }))
        }
      ...
      const filepath = yield* Effect.cached(
        Effect.gen(function* () {
          ...
          if (!config) {
// @effect-diagnostics-next-line unnecessaryFailYieldableError:off
            return yield* Effect.fail(new DuoduoError({ message: String(`unsupported platform for ripgrep: ${platformKey}`), messageZh: String(`ripgrep 不支持当前平台：${platformKey}`), cause: undefined }))
          }
          ...
          const bytes = yield* HttpClientRequest.get(url).pipe(
            ...
            Effect.mapError((cause) => (cause instanceof Error ? cause : new DuoduoError({ message: String(cause), cause: undefined }))),
          )
          if (bytes.byteLength === 0) {
// @effect-diagnostics-next-line unnecessaryFailYieldableError:off
            return yield* Effect.fail(new DuoduoError({ message: String(`failed to download ripgrep from ${url}`), messageZh: String(`从 ${url} 下载 ripgrep 失败`), cause: undefined }))
          }
...
// @effect-diagnostics-next-line unnecessaryFailYieldableError:off
        return yield* Effect.fail(
          Object.assign(new DuoduoError({ message: String(`No such file or directory: '${cwd}'`), messageZh: String(`没有该文件或目录：'${cwd}'`), cause: undefined }), {
            code: "ENOENT",
            errno: -2,
            path: cwd,
          }),
...
// @effect-diagnostics-next-line unnecessaryFailYieldableError:off
      ...
      const tree: Interface["tree"] = Effect.fn("Ripgrep.tree")(function* (input: TreeInput) {
        ...
        for (const file of list) {
          if (file.includes(".duoduo")) continue
