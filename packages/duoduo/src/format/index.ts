...
export class Service extends Context.Service<Service, Interface>()("@duoduocode/Format") {}
    ...
    const state = yield* InstanceState.make(
      Effect.fn("Format.state")(function* (ctx) {
        ...
        if (cfg.formatter !== true) {
          // Built-in formatters are keyed in config by their `name` (e.g. "uv"),
          // which can differ from the module export identifier (e.g. `uvformat`),
          // so resolve built-ins by name to reliably read their `group`.
          const builtInByName = new Map(Object.values(Formatter).map((item) => [item.name, item]))

          // Collect groups whose formatter is disabled, so all formatters sharing a
          // backend (e.g. ruff/uv) are disabled together without hardcoding names.
          const disabledGroups = new Set<string>()
          for (const [name, item] of Object.entries(cfg.formatter)) {
            const group = builtInByName.get(name)?.group
            if (item.disabled && group) disabledGroups.add(group)
          }
          ...
          // Remove any formatter whose backend group was disabled above.
          for (const formatter of Object.values(formatters)) {
            if (formatter.group && disabledGroups.has(formatter.group)) {
              delete formatters[formatter.name]
            }
          }
        }
