...
const DIAGNOSTICS_DOCUMENT_WAIT_TIMEOUT_MS = Flag.DUODUO_LSP_DIAGNOSTICS_TIMEOUT_MS ?? 1_500
const DIAGNOSTICS_FULL_WAIT_TIMEOUT_MS = (Flag.DUODUO_LSP_DIAGNOSTICS_TIMEOUT_MS ?? 1_500) * 2
...
export async function create(input: { serverID: string; server: LSPServer.Handle; root: string; directory: string }) {
    ...
    return new Promise<{ handled: boolean; matched: boolean }>((resolve) => {
      ...
      for (const request of requests) {
        void request
          ...
          .catch(() => {
            // A rejected pull must still decrement pending so the outer
  ...
  async function waitForFreshPush(request: { path: string; version: number; after: number; timeout: number }) {
    ...
    return new Promise<boolean>((resolve) => {
      ...
      unsub = await Bus.subscribe(Event.Diagnostics, (event) => {
  ...
  async function waitForDocumentDiagnostics(request: { path: string; version: number; after?: number }) {
    ...
    while (Date.now() - startedAt < DIAGNOSTICS_FULL_WAIT_TIMEOUT_MS) {
      ...
      const next = await Promise.race([
        ...
        waitForRegistrationChange(Math.min(remaining, DIAGNOSTICS_DOCUMENT_WAIT_TIMEOUT_MS)).then((changed) =>
          changed ? "registration" : ("retry" as const),
        ),
      ])
