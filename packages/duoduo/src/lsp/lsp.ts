...
const filterExperimentalServers = (servers: Record<string, LSPServer.Info>) => {
  if (Flag.DUODUO_EXPERIMENTAL_LSP_TY) {
    if (servers["pyright"]) {
      log.info("LSP server pyright is disabled because DUODUO_EXPERIMENTAL_LSP_TY is enabled")
...
interface State {
  ...
  broken: Map<string, { id: string; root: string }>
...
export interface Interface {
  ...
  readonly warmDirectory: (directory: string) => Effect.Effect<number>
  readonly retry: (root: string, id: string) => Effect.Effect<void>
...
export class Service extends Context.Service<Service, Interface>()("@duoduocode/LSP") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
      ...
      return yield* Effect.promise(async () => {
        ...
        async function schedule(server: LSPServer.Info, root: string, key: string) {
          const handle = await server
            .spawn(root, ctx)
            .then((value) => {
              if (!value) s.broken.set(key, { id: server.id, root })
              return value
            })
            .catch((err) => {
              s.broken.set(key, { id: server.id, root })
              log.error(`Failed to spawn LSP server ${server.id}`, { error: err })
              return undefined
            })

          if (!handle) {
            // 与成功路径(下方 client 就绪后的 publish)对称：失败也要广播，
            // 否则前端（app store 的 lsp 数组、TUI sync）只能靠主动轮询才能看到 error 态
    ...
    const status = Effect.fn("LSP.status")(function* () {
      ...
      for (const info of s.broken.values()) {
        result.push({
          id: info.id,
          name: s.servers[info.id]?.id ?? info.id,
          root: info.root,
          status: "error",
        })
      }
    ...
    const retry = Effect.fn("LSP.retry")(function* (root: string, id: string) {
      const s = yield* InstanceState.get(state)
      s.broken.delete(root + id)
    })
    ...
    const warmDirectory = Effect.fn("LSP.warmDirectory")(function* (directory: string) {
      const s = yield* InstanceState.get(state)
      const extSet = new Set<string>()
      for (const server of Object.values(s.servers)) {
        for (const e of server.extensions) extSet.add(e)
      }
      if (extSet.size === 0) return 0

      // 代表文件收集：每扩展名取 1 个即可触发 schedule（server 自行索引全项目）。
      // 深度 6 覆盖 monorepo 布局（packages/<pkg>/src/*.ts 为 depth 3）；
      // 黑名单排除依赖/构建产物目录；预算上限防超大仓库同步 readdir 卡顿
      // （walk 用同步 IO，跑在 Bun 主线程，必须封顶）。
      // 误收集的扩展名会被 getClients 的 server.extensions + root 双重校验过滤，
      // 不会 spawn 项目里不存在的语言 server。
      ...
      const walk = (dir: string, depth: number) => {
        ...
        for (const ent of entries) {
          ...
          if (ent.isDirectory()) {
            ...
            const ext = path.parse(ent.name).ext.toLowerCase()
            if (!reps.has(ext) && extSet.has(ext)) {
              reps.set(ext, full)
            }
          }
        }
      ...
      for (const file of reps.values()) {
        yield* touchFile(file).pipe(Effect.ignore, Effect.forkDetach)
      }
      // 返回预热文件数：调用方（/lsp/touch 端点）把它回传给前端，
      // 前端据此区分「正在启动」（>0，显示 spinner）与「项目无已知语言」
      // （0，不显示任何指示）。walk 为同步 readdir（预算封顶），
      // 此处返回时 touchFile 仍在后台 detach 运行，不阻塞响应。
      return reps.size
    })
