...
import { Module } from "@duoduo-ai/shared/util/module"
...
export const JDTLS: Info = {
  ...
  async spawn(root) {
    ...
    const dataDir = await fs.mkdtemp(path.join(os.tmpdir(), "duoduo-jdtls-data"))
    return {
      process: spawn(
        java,
        [
...
export const PHPIntelephense: Info = {
  ...
  async spawn(root) {
    // Only a copy the user installed themselves is used. intephense is
    // proprietary: clause 3 of its licence allows an individual end user to
    // run it with an IDE, but clause 5(c) forbids reproducing or distributing
    // it — so we never bundle it and never install it on the user's behalf.
    // PHP code intelligence without it comes from the built-in AST/KG engine.
    const binary = which("intelephense")
    if (!binary) return
