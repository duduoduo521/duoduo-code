...
export const DEPRECATED_PLUGIN_PACKAGES = ["duoduo-openai-codex-auth", "duoduo-copilot-auth"]

export function isDeprecatedPlugin(spec: string) {
  // E-06: match by parsed package name (incl. alias subSpec) instead of naive
  // substring `includes`, which wrongly flagged e.g. `my-duoduo-copilot-auth-wrapper`.
  const parsed = parse(spec)
  if (!parsed) return DEPRECATED_PLUGIN_PACKAGES.includes(spec)
  const names: Array<string | undefined> = [parsed.name ?? undefined]
  if (parsed.type === "alias") {
    names.push((parsed as npa.AliasResult).subSpec?.name ?? undefined)
  }
  return names.some((n) => n !== undefined && DEPRECATED_PLUGIN_PACKAGES.includes(n))
}
...
// [E-04] Local plugin paths (file:// or absolute) must stay within the project
// tree. The npm branch already enforces this via Filesystem.contains; without a
// matching check a `file://` spec could load an arbitrary file anywhere on disk
// as a plugin. Relative specs are resolved against the project (cwd/config dir)
// so they pass; only out-of-tree absolute/file:// specs are rejected.
function assertWithinProject(spec: string, file: string) {
  ...
  if (resolved !== root && !Filesystem.contains(root, resolved)) {
    throw new Error(`本地插件路径越界: ${spec} 必须位于项目目录(${root})内`)
  }
}
...
export async function checkPluginCompatibility(target: string, duoduoVersion: string, pkg?: PluginPackage) {
  // [E-05] Do NOT skip compatibility checks on 0.x versions — they are still
  // subject to `engines.duoduo` ranges. Only an unparseable version bails out.
  if (!semver.valid(duoduoVersion)) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.duoduo
  if (typeof range !== "string") return
  if (!semver.satisfies(duoduoVersion, range)) {
    throw new Error(`Plugin requires duoduo ${range} but running ${duoduoVersion}`)
  }
}
