  ...
  async doGenerate(options: LanguageModelV3CallOptions) {
    ...
    if (choice == null) {
      throw new APICallError({
        message: "No choice returned in response",
    ...
    // reasoning content (Copilot uses `reasoning_text`, DeepSeek/Qwen/Kimi use `reasoning_content`):
  ...
  async doStream(options: LanguageModelV3CallOptions) {
    ...
    return {
      stream: response.pipeThrough(
        new TransformStream<ParseResult<OpenAICompatibleChatChunk>, LanguageModelV3StreamPart>({
          start(controller) {
          ...
          transform(chunk, controller) {
            ...
            if (isFirstChunk) {
              ...
              if (ttfbMs > 0) {
                process.stderr.write(`[LLM] TTFB: ${ttfbMs}ms\n`)
              }
            ...
            if (value.usage != null) {
              ...
              } else if (value.usage?.prompt_cache_hit_tokens != null) {
                usage.promptTokensDetails.cachedTokens = value.usage?.prompt_cache_hit_tokens
              }
            }
            ...
            // Copilot uses `reasoning_text`, DeepSeek/Qwen/Kimi use `reasoning_content`
...
const openaiCompatibleTokenUsageSchema = z
  .object({
    ...
    prompt_tokens_details: z
      ...
      .nullish(),
    // DeepSeek V4 returns the cache-hit count under this private field
    // (value identical to prompt_tokens_details.cached_tokens, different path).
    // Read it as a fallback so cache accounting is non-zero on endpoints that
    // only emit the private field. Harmless for providers that omit it.
    ...
    .array(
      z.object({
        message: z.object({
          ...
          // DeepSeek/Qwen/Kimi reasoning field
...
// The success-branch shape of a streaming chunk. Used to restore type safety on the
// TransformStream input, which is otherwise widened to `any` by the generic error schema.
type OpenAICompatibleChatChunkValue = z.infer<typeof openaiCompatibleChatChunkSchema>

// The error branch of the chunk union carries an `error` field. Its concrete shape is
// supplied by the caller's error schema at runtime, so we type only the `message`
// accessor that this implementation reads.
type OpenAICompatibleChatChunkError = { error: { message: string } }

// Union of the two branches a parsed chunk can take. This replaces the `any` widening
// caused by the generic error schema, restoring type checking on chunk fields.
type OpenAICompatibleChatChunk = OpenAICompatibleChatChunkValue | OpenAICompatibleChatChunkError
