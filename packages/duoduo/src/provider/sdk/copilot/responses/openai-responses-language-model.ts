...
// AI SDK 6: provider options use `.optional()` (only `undefined`, never `null`).
// Verified no caller passes `null` for these fields (see provider/transform.ts which
// always sets `include` to a concrete array, and `instructions` is only read here).
