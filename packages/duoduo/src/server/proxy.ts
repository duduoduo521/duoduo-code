...
const app = (upgrade: UpgradeWebSocket) =>
  new Hono().get(
    "/__workspace_ws",
    upgrade((c) => {
      const url = c.req.header("x-duoduo-proxy-url")
      const queue: Msg[] = []
      ...
      return {
        onOpen(_, ws) {
          ...
          remote.onopen = () => {
            for (const item of queue) remote?.send(item as string | ArrayBuffer | Uint8Array<ArrayBuffer>)
            queue.length = 0
          }
...
export function websocket(
  ...
  next.set("x-duoduo-proxy-url", socket(target))
  for (const [key, value] of new Headers(extra).entries()) {
    next.set(key, value)
  }
