...
export const ProviderApi = HttpApi.make("provider")
  .add(
    HttpApiGroup.make("provider")
      .add(
        ...
        HttpApiEndpoint.post("scanLan", `${root}/scan-lan`, {
          ...
          OpenApi.annotations({
            identifier: "provider.scanLan",
            summary: "Scan LAN for models",
  ...
  .annotateMerge(
    OpenApi.annotations({
      title: "duoduo experimental HttpApi",
      version: "0.0.1",
...
export const providerHandlers = Layer.unwrap(
  Effect.gen(function* () {
    ...
    const scanLan = Effect.fn("ProviderHttpApi.scanLan")(function* () {
      return yield* provider.scanLan()
    })

    return HttpApiBuilder.group(ProviderApi, "provider", (handlers) =>
      handlers
        .handle("list", list)
        .handle("scanLan", scanLan)
        .handle("auth", auth)
