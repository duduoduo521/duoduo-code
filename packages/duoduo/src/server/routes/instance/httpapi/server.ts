...
const Headers = Schema.Struct({
  ...
  "x-duoduo-directory": Schema.optional(Schema.String),
})
...
class Authorization extends HttpApiMiddleware.Service<Authorization>()("@duoduocode/ExperimentalHttpApiAuthorization", {
  error: Unauthorized,
  security: {
...
const normalize = HttpRouter.middleware()(
  Effect.gen(function* () {
    return (effect) =>
      Effect.gen(function* () {
        const _query = yield* HttpServerRequest.schemaSearchParams(Query)
        return yield* effect
      })
  }),
).layer
...
const auth = Layer.succeed(
  ...
  Authorization.of({
    basic: (effect, { credential }) =>
      Effect.gen(function* () {
        if (!Flag.DUODUO_SERVER_PASSWORD) return yield* effect

        const user = Flag.DUODUO_SERVER_USERNAME ?? "duoduo"
...
// @effect-diagnostics-next-line unnecessaryFailYieldableError:off
        ...
        if (Redacted.value(credential.password) !== Flag.DUODUO_SERVER_PASSWORD) {
// @effect-diagnostics-next-line unnecessaryFailYieldableError:off
          return yield* Effect.fail(new Unauthorized({ message: "Unauthorized" }))
        }
        return yield* effect
  ...
  Effect.gen(function* () {
    return (effect) =>
      Effect.gen(function* () {
        ...
        const raw = query.directory || headers["x-duoduo-directory"] || process.cwd()
