...
function process<Def extends Definition>(def: Def, event: Event<Def>, options: { publish: boolean }) {
  ...
  // Dual-DB: projector writes to project-level tables (session/message/part),
  // event log writes to global tables (event/event_sequence).
  // Run the projector in a project-DB transaction first, then record the
  // event in the global-DB transaction.  Effects fire after both commit.

  Database.projectTransaction((projectTx) => {
    projector(projectTx, event.data)
  })
