...
export class Service extends Context.Service<Service, Interface>()("@duoduocode/Worktree") {}
...
// @effect-diagnostics-next-line catchUnfailableEffect:off
    ...
    function cleanDirectory(target: string) {
      // Explicit retry loop instead of fs.rm's maxRetries: Bun's fs.promises
      // implementation does not honour them. On Windows, handles to the
      // worktree directory (watcher threads, fsmonitor shutdown, OS indexer)
      // are released asynchronously after teardown, so deletion fails with
      // EBUSY for up to a few seconds. POSIX keeps a single attempt.
    ...
    const remove = Effect.fn("Worktree.remove")(function* (input: RemoveInput) {
      ...
      if (!entry?.path) {
        ...
        if (directoryExists) {
          // Release the worktree's live Instance (file watcher, runtimes) — on
          // Windows its open handles make the subsequent fs.rm fail with EBUSY.
          yield* Effect.promise(() => Instance.disposeDirectory(directory, 5000)).pipe(
      ...
      // Release the worktree's live Instance before deleting its directory
      // (see the disposeDirectory comment in the branch above).
      const worktreeDir = entry.path
      if (worktreeDir) {
        yield* Effect.promise(() => Instance.disposeDirectory(worktreeDir, 5000)).pipe(
          Effect.catch(() => Effect.succeed(undefined)),
        )
        yield* stopFsmonitor(worktreeDir)
      }
