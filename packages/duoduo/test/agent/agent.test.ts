...
import { projectDataDir } from "../../src/storage/project-dir"
...
function load<A>(dir: string, fn: (svc: Agent.Interface) => Effect.Effect<A, unknown, unknown>) {
  const effect = provideInstance(dir)(Agent.Service.use(fn)).pipe(Effect.provide(Agent.defaultLayer))
  return Effect.runPromise(effect as Effect.Effect<A, unknown, never>)
}

import { disposeAllWithTimeout } from "../lib/dispose"

// Explicit hook timeout: disposeAllWithTimeout races a 10s internal guard, but
// bun's default 5s afterEach window can expire first on Windows (slow FS/GLC),
// failing the hook instead of the disposal resolving on its own.
afterEach(async () => {
  await disposeAllWithTimeout()
}, 15000)

describe.skipIf(!hasRg)("Agent", () => {
  ...
  test("plan agent denies edits except the project data dir plans/*", async () => {
    await using tmp = await tmpdir()
    await Instance.provide({
      directory: tmp.path,
      fn: async () => {
        ...
        // But the plans path under the personal data dir is allowed
        const planFile = path.join(projectDataDir(Instance.worktree), "plans", "foo.md")
        expect(Permission.evaluate("edit", planFile, plan.permission).action).toBe("allow")
      },
    })
  })
  ...
  test("skill directories are allowed for external_directory", async () => {
    await using tmp = await tmpdir({
      git: true,
      init: async (dir) => {
        const skillDir = path.join(dir, ".duoduo", "skill", "perm-skill")
        await Bun.write(
...
`,
    ...
    const home = process.env.DUODUO_TEST_HOME
    process.env.DUODUO_TEST_HOME = tmp.path
    ...
    try {
      await Instance.provide({
        ...
        fn: async () => {
          const build = await load(tmp.path, (svc) => svc.get("build"))
          const skillDir = path.join(tmp.path, ".duoduo", "skill", "perm-skill")
          const target = path.join(skillDir, "reference", "notes.md")
          expect(Permission.evaluate("external_directory", target, build.permission).action).toBe("allow")
        },
      })
    } finally {
      process.env.DUODUO_TEST_HOME = home
    }
  }, { timeout: 15000 })
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        await expect(load(tmp.path, (svc) => svc.defaultAgent())).rejects.toThrow(
          'default agent "explore" is a subagent',
        )
      },
    })
  })
  ...
  test("defaultAgent throws when default_agent points to hidden agent", async () => {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        // oxlint-disable-next-line await-thenable -- vitest matcher is awaitable (thenable)
        await expect(load(tmp.path, (svc) => svc.defaultAgent())).rejects.toThrow(
          'default agent "compaction" is hidden',
        )
      },
    })
  })
