...
// Check if ripgrep is available. Bus.subscribe/publish use AppRuntime which
// includes Ripgrep.defaultLayer. Without rg, the first access downloads ~10 MB
// from GitHub, which times out the test suite when running under CI or without
// network access. The bus tests themselves don't need rg — but the layer that
// provides them does. We skip the entire suite when rg is missing.
import { hasRg } from "../fixture/ripgrep"

const TestEvent = {
  Init: BusEvent.define("test.bus.init", z.object({})),
...
function withInstance(directory: string, fn: () => Promise<void>) {
  return Instance.provide({
    ...
    fn: async () => {
      ...
      await Bus.publish(TestEvent.Init, {})
      await fn()
    },
  })
}

describe.skipIf(!hasRg)("Bus", () => {
  ...
  describe("publish + subscribe", () => {
    test("subscriber is live immediately after subscribe returns", async () => {
      ...
      await withInstance(tmp.path, async () => {
        await Bus.subscribe(TestEvent.Ping, (evt) => {
  ...
  describe("unsubscribe", () => {
    test("unsubscribe stops delivery", async () => {
      ...
      await withInstance(tmp.path, async () => {
        const unsub = await Bus.subscribe(TestEvent.Ping, (evt) => {
          received.push(evt.properties.value)
        })
        await Bun.sleep(10)
        await Bus.publish(TestEvent.Ping, { value: 1 })
