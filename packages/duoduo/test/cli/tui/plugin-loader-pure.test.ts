...
`,
  ...
  const pure = process.env.DUODUO_PURE
  const meta = process.env.DUODUO_PLUGIN_META_FILE
  process.env.DUODUO_PURE = "1"
  process.env.DUODUO_PLUGIN_META_FILE = tmp.extra.meta
  ...
  try {
    ...
    wait.mockRestore()
    if (pure === undefined) {
      delete process.env.DUODUO_PURE
    } else {
      process.env.DUODUO_PURE = pure
    }
    if (meta === undefined) {
      delete process.env.DUODUO_PLUGIN_META_FILE
    } else {
      process.env.DUODUO_PLUGIN_META_FILE = meta
    }
  }
})
