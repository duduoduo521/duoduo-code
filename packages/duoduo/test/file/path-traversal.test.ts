...
describe("Instance.containsPath", () => {
  ...
  test("returns true for path inside worktree but outside directory (monorepo subdirectory scenario)", async () => {
    ...
    await Instance.provide({
      directory: subdir,
      fn: () => {
        // .duoduo at worktree root, but we're running from packages/lib
        expect(Instance.containsPath(path.join(tmp.path, ".duoduo", "state"))).toBe(true)
...
describe("Instance.addAllowedPath", () => {
  test("an approved directory becomes in-bounds without widening to its siblings", async () => {
    await using tmp = await tmpdir({ git: true })
    await using other = await tmpdir({ git: true })
    const projectB = path.join(other.path, "project-b")
    await fs.mkdir(projectB, { recursive: true })

    await Instance.provide({
      directory: tmp.path,
      fn: () => {
        // Before approval the cross-project path is external — this is what
        // triggers the `external_directory` prompt in the first place.
        expect(Instance.containsPath(path.join(projectB, "src.ts"))).toBe(false)

        Instance.addAllowedPath(projectB)

        expect(Instance.containsPath(path.join(projectB, "src.ts"))).toBe(true)
        // Approving one directory must not hand over its parent or peers.
        expect(Instance.containsPath(path.join(other.path, "project-c", "x.ts"))).toBe(false)
        expect(Instance.containsPath("/etc/passwd")).toBe(false)
      },
    })
  })
  ...
  test("is idempotent", async () => {
    ...
    await Instance.provide({
      directory: tmp.path,
      fn: () => {
        Instance.addAllowedPath(other.path)
        const list = Instance.addAllowedPath(other.path)
        expect(list.filter((p) => p === other.path)).toHaveLength(1)
      },
    })
  })

  test("accepts an explicit context, for callers outside the instance ALS", async () => {
    await using tmp = await tmpdir({ git: true })
    await using other = await tmpdir({ git: true })

    // `Permission.reply` runs from an HTTP handler, so it resolves the context
    // via InstanceRef and passes it in rather than relying on the ALS.
    const ctx = await Instance.provide({
      directory: tmp.path,
      fn: () => Instance.current,
    })

    Instance.addAllowedPath(other.path, ctx)
    expect(Instance.containsPath(path.join(other.path, "file.ts"), ctx)).toBe(true)
  })
})
