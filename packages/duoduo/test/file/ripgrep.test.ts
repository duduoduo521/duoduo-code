...
describe.skipIf(!hasRg)("file.ripgrep", () => {
  test("defaults to include hidden", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        await Bun.write(path.join(dir, "visible.txt"), "hello")
        await fs.mkdir(path.join(dir, ".duoduo"), { recursive: true })
        await Bun.write(path.join(dir, ".duoduo", "thing.json"), "{}")
      },
    })
    ...
    expect(files.includes(path.join(".duoduo", "thing.json"))).toBe(true)
  })
  ...
  test("hidden false excludes hidden", async () => {
    ...
    expect(files.includes("visible.txt")).toBe(true)
    expect(files.includes(path.join(".duoduo", "thing.json"))).toBe(false)
  })
