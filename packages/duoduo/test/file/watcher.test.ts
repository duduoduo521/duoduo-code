...
// On Windows, the @parcel/watcher windows backend has timing issues with .git/* events
// and short-lived test subscriptions, even though it works correctly in production.
const describeWatcher =
  FileWatcher.hasNativeBinding() && !process.env.CI && process.platform !== "win32" ? describe : describe.skip
...
const watcherConfigLayer = ConfigProvider.layer(
  ConfigProvider.fromUnknown({
    DUODUO_EXPERIMENTAL_FILEWATCHER: "true",
    DUODUO_EXPERIMENTAL_DISABLE_FILEWATCHER: "false",
  }),
)
...
function listen(directory: string, check: (evt: WatcherEvent) => boolean, hit: (evt: WatcherEvent) => void) {
  ...
  // FileWatcher emits via GlobalBus (not Bus.publish), so we listen on GlobalBus directly.
...
describeWatcher("FileWatcher", () => {
  // FileWatcher integration tests involve native file watching + debounce (100ms)
  // and can take several seconds per test. Set a generous timeout.
  afterEach(async () => {
