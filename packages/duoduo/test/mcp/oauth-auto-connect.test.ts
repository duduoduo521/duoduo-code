...
test("state() generates a new state when none is saved", async () => {
  ...
  // Use a unique name per test run to avoid state leakage from previous runs
  // (the auth db is persisted across test invocations).
  const uniqueName = `test-state-gen-${crypto.randomUUID()}`
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const entryBefore = await Effect.runPromise(
        McpAuth.Service.use((auth) => auth.get(uniqueName)).pipe(Effect.provide(McpAuth.defaultLayer)),
      )
