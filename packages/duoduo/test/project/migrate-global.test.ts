...
// Sessions live in the per-project DB (`<dbDir>/.duoduo/duoduo.db`). `dbDir`
// selects WHICH project DB to write to; `dir` is the session's `directory`
// column value (which the migration matches against the project worktree).
// These were the same under the old global-DB layout but are independent now.
function seed(opts: { id: SessionID; dbDir: string; dir: string; project: ProjectID }) {
  const now = Date.now()
  Database.useProjectAt(opts.dbDir, (db) =>
    db
      .insert(SessionTable)
      .values({
        id: opts.id,
...
function readSession(dbDir: string, id: SessionID) {
  return Database.useProjectAt(dbDir, (db) => db.select().from(SessionTable).where(eq(SessionTable.id, id)).get())
}
...
describe("migrateFromGlobal", () => {
  test("migrates global sessions on first project creation", async () => {
    ...
    await $`git config user.email "test@duoduo.test"`.cwd(tmp.path).quiet()
    ...
    const id = uid()
    seed({ id, dbDir: tmp.path, dir: tmp.path, project: ProjectID.global })
    ...
    const row = readSession(tmp.path, id)
  ...
  test("does not claim sessions with empty directory", async () => {
    ...
    const id = uid()
    seed({ id, dbDir: tmp.path, dir: "", project: ProjectID.global })
  ...
  test("does not steal sessions from unrelated directories", async () => {
    ...
    const id = uid()
    seed({ id, dbDir: tmp.path, dir: "/some/other/dir", project: ProjectID.global })
