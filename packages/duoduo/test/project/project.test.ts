...
describe("Project.fromDirectory", () => {
  test("should handle git repository with no commits", async () => {
    ...
    const { project } = await run((svc) => svc.fromDirectory(tmp.path, undefined, { create: true }))
    ...
    const duoduoFile = path.join(tmp.path, ".git", "duoduo")
    expect(await Bun.file(duoduoFile).exists()).toBe(false)
  })
  ...
  test("should handle git repository with commits", async () => {
    ...
    // No file is written into the project tree; the git id cache now lives in
    // the data directory, keyed by the git common dir path.
    const duoduoFile = path.join(tmp.path, ".git", "duoduo")
    expect(await Bun.file(duoduoFile).exists()).toBe(false)

    // The cache is written to the data dir instead.
    if (Global.Path.data) {
      const cache = path.join(Global.Path.data, "git-id", projectId(path.join(tmp.path, ".git")))
      expect(await Bun.file(cache).exists()).toBe(true)
    }
  })

  test("returns a directory-scoped project for non-git directory", async () => {
    await using tmp = await tmpdir()
    const { project } = await run((svc) => svc.fromDirectory(tmp.path, undefined, { create: true }))
    // Non-git directories are their own project (id derived from the path), not the
    // shared global project — changed intentionally in 46aa494a ("项目体验相关问题修复").
  ...
  test("derives stable project ID from root commit", async () => {
    await using tmp = await tmpdir({ git: true })
    const { project: a } = await run((svc) => svc.fromDirectory(tmp.path, undefined, { create: true }))
    const { project: b } = await run((svc) => svc.fromDirectory(tmp.path, undefined, { create: true }))
...
describe("Project.fromDirectory git failure paths", () => {
  ...
  test("handles show-toplevel failure gracefully", async () => {
    ...
    const { project, sandbox } = await run((svc) => svc.fromDirectory(tmp.path, undefined, { create: true }), layer)
    expect(project.worktree).toBe(tmp.path)
...
describe("Project.fromDirectory with worktrees", () => {
  test("should set worktree to root when called from root", async () => {
    ...
    const { project, sandbox } = await run((svc) => svc.fromDirectory(tmp.path, undefined, { create: true }))
  ...
  test("should set worktree to root when called from a worktree", async () => {
    ...
    try {
      ...
      const { project, sandbox } = await run((svc) => svc.fromDirectory(worktreePath, undefined, { create: true }))
  ...
  test("worktree should share project ID with main repo", async () => {
    ...
    const { project: main } = await run((svc) => svc.fromDirectory(tmp.path, undefined, { create: true }))
    ...
    try {
      ...
      const { project: wt } = await run((svc) => svc.fromDirectory(worktreePath, undefined, { create: true }))

      expect(wt.id).toBe(main.id)

      // No cache file is written into the project tree (neither the worktree's
      // `.git` file nor the common `.git` dir); it lives in the data directory.
      const inTree = path.join(tmp.path, ".git", "duoduo")
      expect(await Bun.file(inTree).exists()).toBe(false)
  ...
  test("separate clones of the same repo should share project ID", async () => {
    ...
    try {
      ...
      const { project: a } = await run((svc) => svc.fromDirectory(tmp.path, undefined, { create: true }))
      const { project: b } = await run((svc) => svc.fromDirectory(clone, undefined, { create: true }))
  ...
  test("should accumulate multiple worktrees in sandboxes", async () => {
    ...
    try {
      ...
      await run((svc) => svc.fromDirectory(worktree1, undefined, { create: true }))
      const { project } = await run((svc) => svc.fromDirectory(worktree2, undefined, { create: true }))
...
describe("Project.fromDirectory with bare repos", () => {
  test("worktree from bare repo should cache in bare repo, not parent", async () => {
    ...
    try {
      ...
      const { project } = await run((svc) => svc.fromDirectory(worktreePath, undefined, { create: true }))

      expect(project.id).not.toBe(ProjectID.global)
      expect(project.worktree).toBe(barePath)

      const inTree = path.join(barePath, "duoduo")
      const wrongCache = path.join(parentDir, ".git", "duoduo")

      expect(await Bun.file(inTree).exists()).toBe(false)
      expect(await Bun.file(wrongCache).exists()).toBe(false)
      if (Global.Path.data) {
        const cache = path.join(Global.Path.data, "git-id", projectId(barePath))
        expect(await Bun.file(cache).exists()).toBe(true)
      }
    } finally {
  ...
  test("different bare repos under same parent should not share project ID", async () => {
    ...
    try {
      ...
      const cacheA = path.join(bareA, "duoduo")
      const cacheB = path.join(bareB, "duoduo")
      const wrongCache = path.join(parentDir, ".git", "duoduo")

      expect(await Bun.file(cacheA).exists()).toBe(false)
      expect(await Bun.file(cacheB).exists()).toBe(false)
      expect(await Bun.file(wrongCache).exists()).toBe(false)
      if (Global.Path.data) {
        expect(await Bun.file(path.join(Global.Path.data, "git-id", projectId(bareA))).exists()).toBe(true)
        expect(await Bun.file(path.join(Global.Path.data, "git-id", projectId(bareB))).exists()).toBe(true)
      }
    } finally {
