...
// Skip on Windows — @parcel/watcher's windows backend does not reliably emit
// events for .git/HEAD changes within reasonable test timeouts. The behavior
// works correctly in production usage with longer-lived subscriptions.
const describeVcs =
...
async function withVcs(directory: string, body: () => Promise<void>) {
  return Instance.provide({
    ...
    fn: async () => {
      ...
      // Allow file watcher to fully establish subscriptions.
      // Windows @parcel/watcher backend needs more time than Linux/macOS.
      await Bun.sleep(process.platform === "win32" ? 2000 : 500)
      await body()
    },
  })
}
