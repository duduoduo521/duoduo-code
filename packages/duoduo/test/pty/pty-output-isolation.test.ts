...
// The tests only need a process that echoes written input back through the
// PTY. `cat` does this on POSIX but does not exist on Windows, where bun-pty
// (ConPTY) instead echoes typed input in an interactive `cmd /q` session.
const echoCommand =
  process.platform === "win32" ? { command: "cmd", args: ["/q"] } : { command: "cat", args: [] }
