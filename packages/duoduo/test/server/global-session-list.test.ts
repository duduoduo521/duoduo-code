...
// Opening a project persists its row (matching the production `project.open`
// route, which calls fromDirectory with { create: true }). session.listGlobal
// enumerates projects via Project.list(), so the row must exist for its
// sessions to be discoverable. Instance.provide alone does not persist.
