...
describe("paramToAttributeKey", () => {
  ...
  test("namespaces non-ID params under duoduo.", () => {
    ...
    expect(paramToAttributeKey("slug")).toBe("duoduo.slug")
  })
})
...
describe("requestAttributes", () => {
  ...
  test("emits OTel-style <domain>.id for ID-shaped route params", () => {
    ...
    expect(attrs["duoduo.sessionID"]).toBeUndefined()
    expect(attrs["duoduo.messageID"]).toBeUndefined()
    expect(attrs["duoduo.partID"]).toBeUndefined()
  })
  ...
  test("namespaces non-ID params under duoduo. (e.g. mcp :name)", () => {
    const attrs = requestAttributes(
      fakeContext("POST", "http://localhost/mcp/exa/connect", {
        name: "exa",
      }),
    )
    expect(attrs["duoduo.name"]).toBe("exa")
