...
function run<A, E>(fx: Effect.Effect<A, E, SessionNs.Service | unknown>) {
  return Effect.runPromise(fx.pipe(Effect.provide(SessionNs.defaultLayer)) as unknown as Effect.Effect<A, E, never>)
}
...
function createModel(opts: {
  ...
  return {
    ...
    api: { npm: opts.npm ?? "@ai-sdk/openai-compatible" },
    options: {},
  } as Provider.Model
}
...
function runtime(result: "continue" | "compact", provider = ProviderTest.fake(), config = Config.defaultLayer) {
  const bus = Bus.layer
...
// The `it.live` tests drive the full prompt loop, delegated to the Rust
// smart-layer; they require DUO_SMART_LAYER_URL. Gated, not deleted (Rust has no
// equivalent loop coverage). The plain `test(...)` unit tests below remain ungated.
...
describe("session.compaction.isOverflow", () => {
  ...
  live(
    ...
    provideTmpdirInstance(() =>
      Effect.gen(function* () {
        ...
        // effectiveUsable = (200K - 32K) * 0.65 = 109_200
        // count must be < 109_200 to not overflow
        const tokens = { input: 90_000, output: 10_000, reasoning: 0, cache: { read: 0, write: 0 } }
  ...
  live(
    ...
    provideTmpdirInstance(() =>
      Effect.gen(function* () {
        const compact = yield* SessionCompaction.Service
        const model = createModel({ context: 400_000, input: 272_000, output: 128_000 })
        // usable = 272K - min(20K, 32K) = 252K; effectiveUsable = 252K * 0.65 = 163_800
        // count must be < 163_800 to not overflow
        const tokens = { input: 140_000, output: 10_000, reasoning: 0, cache: { read: 5_000, write: 0 } }
        expect(yield* compact.isOverflow({ tokens, model })).toBe(false)
      }),
    ),
  )
  ...
  //
  // The three cases below previously documented a real asymmetry: when a model
  // declared limit.input, isOverflow() used it verbatim as the usable budget and
  // reserved nothing for output, so a conversation sitting 2K under the input
  // limit never compacted and overflowed on the next turn. That is fixed —
  // these now run in the normal (non-live) suite as regression coverage.
  // Upstream references: #10634, #8089, #11086, #12621; PRs: #6875, #12924.

  it.effect(
    "reserves output headroom when limit.input is set (compacts near the boundary)",
    provideTmpdirInstance(() =>
      Effect.gen(function* () {
        const compact = yield* SessionCompaction.Service
  ...
  it.effect(
    "reserves output headroom when limit.input is absent",
    provideTmpdirInstance(() =>
      Effect.gen(function* () {
        const compact = yield* SessionCompaction.Service
  ...
  it.effect(
    "treats models with and without limit.input symmetrically",
    provideTmpdirInstance(() =>
      Effect.gen(function* () {
        const compact = yield* SessionCompaction.Service
  ...
  live(
    ...
    provideTmpdirInstance(() =>
      Effect.gen(function* () {
        const compact = yield* SessionCompaction.Service
        // When limit.context=0, resolveContextWindow falls back to 128K safe default.
        // effectiveUsable = (128K - 32K) * 0.65 = 62_400
        // count must be < 62_400 to not overflow
        const model = createModel({ context: 0, output: 32_000 })
        const tokens = { input: 30_000, output: 10_000, reasoning: 0, cache: { read: 0, write: 0 } }
        expect(yield* compact.isOverflow({ tokens, model })).toBe(false)
      }),
    ),
  )
...
describe("session.compaction.process", () => {
  ...
  test("publishes compacted event on continue", async () => {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const done = defer()
        let seen = false
        const rt = runtime("continue", wide())
      ...
      fn: async () => {
        ...
        try {
          ...
          if (summary?.info.role === "assistant") {
            ...
            expect(JSON.stringify(summary.info.error)).toContain("当前会话过大")
          }
        } finally {
          await rt.dispose()
        }
  ...
  test("persists tail_start_id for retained recent turns", async () => {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const rt = runtime("continue", wide(), cfg({ tail_turns: 2, preserve_recent_tokens: 10_000 }))
        try {
          const msgs = await svc.messages({ sessionID: session.id })
  ...
  test("shrinks retained tail to fit preserve token budget", async () => {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const rt = runtime("continue", wide(), cfg({ tail_turns: 2, preserve_recent_tokens: 100 }))
        try {
          const msgs = await svc.messages({ sessionID: session.id })
          const parent = msgs.at(-1)?.info.id
          expect(parent).toBeTruthy()
  ...
  test("counts data URL length when estimating retained tail media (estimateFromParts)", async () => {
    await using tmp = await tmpdir({ git: true })
    const stub = llm()
    let captured = ""
    stub.push(
  ...
  test("stops promptly when aborted during retry backoff", async () => {
    const stub = llm()
    const ready = defer()
    stub.push(
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        try {
          ...
          // Waiting for the retry status is a correctness wait, not a latency
          // assertion — give it generous headroom so a loaded machine does not
          ...
          abort.abort()
          // The point of this test is that aborting interrupts the 10s
          // `retry-after-ms` backoff. So the ceiling only needs to be far below
          // 10s: a tight ms budget asserted nothing except scheduler noise.
          const limit = 3000
          const result = await Promise.race([
            run.then((value) => ({ kind: "done" as const, value, ms: Date.now() - start })),
            wait(limit).then(() => ({ kind: "timeout" as const })),
          ])
  ...
  test("does not leave a summary assistant when aborted before processor setup", async () => {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        let run: Promise<"continue" | "stop"> | undefined
        try {
          // Abort BEFORE starting so the interrupt is guaranteed to land ahead
          // of any processor setup. Sleeping a fixed 50ms and hoping the run is
          // still setting up raced with the machine's speed: on a fast run the
          // compaction finished and the result was "continue" instead of "stop".
          abort.abort()
          run = rt
            .runPromiseExit(
              SessionCompaction.Service.use((svc) =>
  ...
  test("ignores previous summaries when sizing the retained tail", async () => {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const rt = runtime("continue", wide(), cfg({ tail_turns: 2, preserve_recent_tokens: 500 }))
        try {
          const msgs = await svc.messages({ sessionID: session.id })
          const parent = msgs.at(-1)?.info.id
          expect(parent).toBeTruthy()
