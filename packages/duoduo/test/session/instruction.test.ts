...
describe("Instruction.resolve", () => {
  ...
  test("fetches remote instructions from config URLs via HttpClient", async () => {
    const server = Bun.serve({
      port: 0,
      fetch: () => new Response("# Remote Instructions", { status: 200 }),
    })
    ...
    await using tmp = await tmpdir({
      init: async (dir) => {
        await Bun.write(
          ...
          JSON.stringify({ $schema: APP_CONFIG_SCHEMA, instructions: [url] }),
        )
      },
    })
      ...
      await Instance.provide({
        ...
        fn: () =>
          run(
            Instruction.Service.use((svc) =>
              Effect.gen(function* () {
                // Non-URL entries land in systemPaths(); http(s) entries are
                // fetched separately and appended by system().
                const paths = yield* svc.systemPaths()
                expect(paths.has(url)).toBe(false)

                const rules = yield* svc.system()
                expect(rules).toContain(`Instructions from: ${url}\n# Remote Instructions`)
              }),
            ),
          ),
      })
...
describe("Instruction.system", () => {
  test("loads both project and global AGENTS.md when both exist", async () => {
    const originalConfigDir = process.env["DUODUO_CONFIG_DIR"]
    delete process.env["DUODUO_CONFIG_DIR"]
    ...
    try {
      ...
      if (originalConfigDir === undefined) {
        ...
        process.env["DUODUO_CONFIG_DIR"] = originalConfigDir
      }
    }
  })
})

describe("Instruction.systemPaths DUODUO_CONFIG_DIR", () => {
  let originalConfigDir: string | undefined

  beforeEach(() => {
    originalConfigDir = process.env["DUODUO_CONFIG_DIR"]
  })
  ...
  test("prefers DUODUO_CONFIG_DIR AGENTS.md over global when both exist", async () => {
    ...
    process.env["DUODUO_CONFIG_DIR"] = profileTmp.path
    const originalGlobalConfig = Global.Path.config
    ;(Global.Path as { config: string }).config = globalTmp.path
  ...
  test("falls back to global AGENTS.md when DUODUO_CONFIG_DIR has no AGENTS.md", async () => {
    await using profileTmp = await tmpdir()
    await using globalTmp = await tmpdir({
      init: async (dir) => {
        await Bun.write(path.join(dir, "AGENTS.md"), "# Global Instructions")
  ...
  test("uses global AGENTS.md when DUODUO_CONFIG_DIR is not set", async () => {
    await using globalTmp = await tmpdir({
      init: async (dir) => {
        await Bun.write(path.join(dir, "AGENTS.md"), "# Global Instructions")
      },
