...
// Use AppRuntime (full layer stack including SystemPrompt) for stream tests.
// The sharedMemoMap in makeRuntime prevents mock layers from overriding
// cached builds, so we rely on the production layer composition instead.
async function drain(input: LLM.StreamInput) {
  return AppRuntime.runPromise(
    Effect.gen(function* () {
      const svc = yield* LLM.Service
      return yield* svc.stream(input).pipe(Stream.runDrain)
    }),
  )
}
...
// LLM stream now delegates to the Rust smart-layer (DUO_SMART_LAYER_URL).
// These tests verified the old TS-direct HTTP path which no longer exists.
// The Rust provider/stream logic lives in crates/agent-executor/src/llm.rs
// (e.g. call_llm_stream / call_llm_stream_with_fallback / build_request_json).
describe.skip("session.llm.stream", () => {
  test("sends temperature, tokens, and reasoning options for openai-compatible models", async () => {
    ...
    const modelID = "custom-reasoning"
          ...
          JSON.stringify({
            ...
            provider: {
              [providerID]: {
                ...
                api: "https://api.custom-llm.example.com/v1",
                models: {
                  [modelID]: {
                    id: modelID,
  ...
  test("sends responses API payload for OpenAI models", async () => {
    ...
    await Instance.provide({
      directory: tmp.path,
      fn: async () => {
        const resolved = await getModel(ProviderID.make("openai"), ModelID.make(modelID))
        ...
        console.log(
          "DEBUG resolved:",
          JSON.stringify(
            { id: resolved.id, api: resolved.api, variants: resolved.variants, release_date: resolved.release_date },
            null,
            2,
          ),
        )
        ...
        expect(body.model).toBe(resolved.api.id)
        expect(body.stream).toBe(true)
        // Responses API encodes reasoning differently from Chat Completions;
        // variant "high" populates include: ["reasoning.encrypted_content"]
        const includeList = body.include as string[] | undefined
        expect(includeList).toContain("reasoning.encrypted_content")
  ...
  test("sends messages API payload for Anthropic Compatible models", async () => {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        expect(body.temperature).toBe(0.4)
        // top_p is not sent by Anthropic SDK when ProviderTransform.topP() returns undefined
        // This model ID (claude-sonnet-4-20250514) does not match any hardcoded pattern in topP()
      },
    })
  })
        ...
        const input = [
          {
            info: {
              ...
              time: { created: 1 },
              agent: "gentleman",
              model: { providerID, modelID, variant: "max" },
            },
            parts: [
              {
                id: "p_user",
  ...
  test("sends Google API payload for Gemini models", async () => {
    ...
    const pathSuffix = `/v1beta/models/${modelID}:streamGenerateContent`
