...
describe("session.message-v2.toModelMessage", () => {
  ...
  test("preserves assistant messages with non-abort errors (network/API)", async () => {
    const assistantID = "m-assistant"
    ...
    const input: MessageV2.WithParts[] = [
      {
        info: assistantInfo(
          assistantID,
          "m-parent",
          new MessageV2.APIError({ message: "boom", isRetryable: true }).toObject(),
        ),
        parts: [
          {
            ...basePart(assistantID, "a1"),
    ...
    // Non-abort errors (network/API) should preserve the assistant message
  ...
  test("skips aborted assistant messages, preserving only synthetic gist", async () => {
  ...
  test("preserves synthetic gist from aborted messages as user message", async () => {
    const assistantID = "m-assistant"
    ...
    const input: MessageV2.WithParts[] = [
      {
        ...
        parts: [
          ...
          },
          {
            ...basePart(assistantID, "a2"),
            type: "text",
            text: "Progress: completed file search",
            synthetic: true,
          } as MessageV2.Part & { synthetic: boolean },
        ] as MessageV2.Part[],
      },
    ]
    ...
    // Aborted message with non-synthetic text part: hasCompletedParts=true,
    // so the message is preserved as an assistant message with interruption prefix
    // plus the non-synthetic text. The synthetic gist is NOT extracted as a user message
    // in this code path — it's only extracted when hasCompletedParts=false.
    expect(await MessageV2.toModelMessages(input, model)).toStrictEqual([
      {
        role: "assistant",
        content: [
...
describe("session.message-v2.fromError", () => {
    ...
    cases.forEach((item) => {
      ...
      expect(result).toStrictEqual({
        ...
        data: {
          ...
          isRetryable: item.isRetryable ?? false,
          responseBody: JSON.stringify(input),
        },
      })
    })
  ...
  test("detects context overflow from Xunfei 10012 + InvalidParameter", () => {
    // Exact error pattern from Xunfei Spark when context is exceeded
    const error = new APICallError({
      message: "Xunfei request failed with Sid: cht000e9545 code: 10012, msg: EngineInternalError:error",
      url: "https://example.com",
      requestBodyValues: {},
      statusCode: 400,
      responseHeaders: { "content-type": "application/json" },
  ...
  test("detects context overflow from Xunfei Range of input length in error message", () => {
    const cases = [
      "InternalError.Algo.InvalidParameter: Range of input length should be [1, 202745]",
  ...
  test("does NOT classify Xunfei 10012 without overflow keywords as context overflow", () => {
    const error = new APICallError({
      message:
        "Xunfei request failed with code: 10012, msg: EngineInternalError: The system is busy, please try again later",
      url: "https://example.com",
      requestBodyValues: {},
      statusCode: 400,
      responseHeaders: { "content-type": "application/json" },
    ...
    const result = MessageV2.fromError(error, { providerID })
    // Should be classified as quota_exceeded (retryable), not context_overflow
  ...
  test("classifies ZlibError from fetch as retryable APIError", () => {
    const zlibError = new Error(
      'ZlibError fetching "https://duoduo.cloudflare.dev/anthropic/messages". For more information, pass `verbose: true` in the second argument to fetch()',
    )
