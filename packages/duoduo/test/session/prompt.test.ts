...
import { SessionCompletion } from "../../src/session/completion"
...
import { GraphIndexStatus } from "../../src/project/graph-index-status"
...
import sessionProjectors from "../../src/session/projectors"
...
async function waitForRunningTool(sessionID: SessionID, timeoutMs = 5000) {
  ...
  throw new Error("timed out waiting for running tool")
}
...
function makeHttp() {
  ...
  return Layer.mergeAll(
    ...
    SessionPrompt.layer.pipe(
      ...
      Layer.provide(SessionCompletion.defaultLayer),
      Layer.provideMerge(deps),
    ),
...
// The prompt loop is delegated to the Rust smart-layer (`delegateToRustRunLoop`),
// so these tests exercise the full TS→Rust path and require a running smart-layer
// (`DUO_SMART_LAYER_URL`). They are gated (not deleted): Rust currently has NO
// equivalent deterministic loop coverage, so they are preserved as integration
// tests that run when the smart-layer is available, and skipped in the plain unit
// suite to keep it green.
const runRunLoop = !!process.env.DUO_SMART_LAYER_URL
const live = it.live.skipIf(!runRunLoop)
const unix = process.platform !== "win32" ? live : it.live.skip
...
live("loop exits immediately when last assistant has stop finish", () =>
  provideTmpdirServer(
    Effect.fnUntraced(function* ({ llm }) {
      ...
      const result = yield* prompt.runAgentTurn({ sessionID: chat.id })
...
live("static loop returns assistant text through local provider", () =>
  provideTmpdirServer(
    Effect.fnUntraced(function* ({ llm }) {
      const prompt = yield* SessionPrompt.Service
      const sessions = yield* Session.Service
      ...
      const result = yield* prompt.runAgentTurn({ sessionID: session.id })
      expect(result.info.role).toBe("assistant")
...
live("static loop consumes queued replies across turns", () =>
  provideTmpdirServer(
    Effect.fnUntraced(function* ({ llm }) {
      const prompt = yield* SessionPrompt.Service
      const sessions = yield* Session.Service
      ...
      const first = yield* prompt.runAgentTurn({ sessionID: session.id })
      ...
      const second = yield* prompt.runAgentTurn({ sessionID: session.id })
...
live("loop continues when finish is tool-calls", () =>
  provideTmpdirServer(
    Effect.fnUntraced(function* ({ llm }) {
      const prompt = yield* SessionPrompt.Service
      const sessions = yield* Session.Service
...
live("glob tool keeps instance context during prompt runs", () =>
  provideTmpdirServer(
    ({ dir, llm }) =>
      Effect.gen(function* () {
        const prompt = yield* SessionPrompt.Service
...
live("loop continues when finish is stop but assistant has tool parts", () =>
  provideTmpdirServer(
    Effect.fnUntraced(function* ({ llm }) {
      const prompt = yield* SessionPrompt.Service
      const sessions = yield* Session.Service
...
live("failed subtask preserves metadata on error tool state", () =>
  provideTmpdirServer(
    Effect.fnUntraced(function* ({ llm }) {
      const prompt = yield* SessionPrompt.Service
      const sessions = yield* Session.Service
  ...
  () =>
    provideTmpdirServer(
      Effect.fnUntraced(function* ({ llm }) {
        ...
        const fiber = yield* prompt.runAgentTurn({ sessionID: chat.id }).pipe(Effect.forkChild)
  ...
  () =>
    provideTmpdirServer(
      Effect.fnUntraced(function* ({ llm }) {
        ...
        const a = yield* prompt.runAgentTurn({ sessionID: chat.id }).pipe(Effect.forkChild)
        yield* llm.wait(1)
        const b = yield* prompt.runAgentTurn({ sessionID: chat.id }).pipe(Effect.forkChild)
...
live("concurrent loop callers get same result", () =>
  provideTmpdirInstance(
    (_dir) =>
      Effect.gen(function* () {
        ...
        const [a, b] = yield* Effect.all([prompt.runAgentTurn({ sessionID: chat.id }), prompt.runAgentTurn({ sessionID: chat.id })], {
...
live("assertNotBusy succeeds when idle", () =>
  provideTmpdirInstance(
    (_dir) =>
      Effect.gen(function* () {
        const run = yield* SessionRunState.Service
  ...
  () =>
    provideTmpdirServer(
      Effect.fnUntraced(function* ({ llm }) {
        ...
        const loop = yield* prompt.runAgentTurn({ sessionID: chat.id }).pipe(Effect.forkChild)
        yield* Effect.sleep(50)
        ...
        (_dir) =>
          Effect.gen(function* () {
            ...
            const sh = yield* prompt
              ...
              .pipe(Effect.forkChild)
            yield* Effect.promise(() => waitForRunningTool(chat.id))
    ...
    provideTmpdirServer(
      ({ dir, llm }) =>
        Effect.gen(function* () {
          ...
          yield* llm.tool("bash", {
            command:
              'yes "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx" | head -n 4000; sleep 30',
            description: "Print many lines",
            timeout: 30_000,
            workdir: path.resolve(dir),
          })

          const run = yield* prompt.runAgentTurn({ sessionID: chat.id }).pipe(Effect.forkChild)
          yield* llm.wait(1)
...
live("does not fail the prompt when a file part is missing", () =>
  provideTmpdirInstance(
    (dir) =>
      Effect.gen(function* () {
        const prompt = yield* SessionPrompt.Service
...
live("keeps stored part order stable when file resolution is async", () =>
  provideTmpdirInstance(
    (dir) =>
      Effect.gen(function* () {
        const prompt = yield* SessionPrompt.Service
...
live("handles filenames with # character", () =>
  provideTmpdirInstance(
    (dir) =>
      Effect.gen(function* () {
...
live("does not loop empty assistant turns for a simple reply", () =>
  provideTmpdirServer(
    Effect.fnUntraced(function* ({ llm }) {
      const prompt = yield* SessionPrompt.Service
      const sessions = yield* Session.Service
      ...
      yield* llm.text("packages/duoduo/src/session/processor.ts")
...
live("applies agent variant only when using agent model", () =>
  provideTmpdirInstance(
    (_dir) =>
      Effect.gen(function* () {
        const prompt = yield* SessionPrompt.Service
        ...
        const other = yield* prompt.prompt({
          sessionID: session.id,
          agent: "build",
          model: { providerID: ProviderID.make("duoduo"), modelID: ModelID.make("kimi-k2.5-free") },
          noReply: true,
          parts: [{ type: "text", text: "hello" }],
        })
...
// ② Coding-standards injection (A-class enhancement): the local stream path
// (llm.ts) must include the AGENTS.md family guidance in the system prompt.
live(
  "injects local coding standards (AGENTS.md) into the stream system prompt",
  () =>
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          // Write a project AGENTS.md so Instruction.system() surfaces it.
          yield* Effect.promise(() =>
            Bun.write(
              path.join(dir, "AGENTS.md"),
              "# Project Rules\n- STRICTMODE_RULE_XYZ: always use strict mode\n",
            ),
          )
          ...
          // The system prompt assembled by llm.ts is sent to the (mock) LLM;
          // assert the coding-standards marker made it through.
          const inputs = yield* llm.inputs
          const last = inputs.at(-1)
          const serialized = JSON.stringify(last?.messages ?? [])
          expect(serialized).toContain("STRICTMODE_RULE_XYZ")
        }),
      { git: true, config: cfg },
    ),
  30_000,
