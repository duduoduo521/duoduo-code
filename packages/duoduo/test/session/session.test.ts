...
// Initialize AppRuntime so Bus.subscribe() works.
// Must happen inside Instance.provide because Bus.subscribeCallback depends on InstanceState.
async function initAppRuntime() {
  await Instance.provide({
    directory: projectRoot,
    fn: () => Bus.publish(BusEvent.define("test.init", z.object({})), {}),
  })
}

beforeAll(initAppRuntime)
  ...
  test("should emit session.created event when session is created", async () => {
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const unsub = await Bus.subscribe(SessionNs.Event.Created, (event) => {
          eventReceived = true
  ...
  test("session.created event should be emitted before session.updated", async () => {
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const unsubCreated = await Bus.subscribe(SessionNs.Event.Created, () => {
          events.push("created")
        })

        const unsubUpdated = await Bus.subscribe(SessionNs.Event.Updated, () => {
          events.push("updated")
        })
    ...
    async () => {
      await Instance.provide({
        ...
        fn: async () => {
          ...
          const unsub = await Bus.subscribe(MessageV2.Event.PartUpdated, (event) => {
...
describe("Session", () => {
  test("remove works without an instance", async () => {
    ...
    // remove() resolves the project DB via Instance context, so it must run
    // inside Instance.provide — same shape as the create() call above.
    // oxlint-disable-next-line await-thenable -- vitest matcher is awaitable (thenable)
    await expect(async () => {
      await Instance.provide({ directory: tmp.path, fn: () => remove(info.id) })
    }).not.toThrow()

    let missing = false
    await Instance.provide({ directory: tmp.path, fn: () => get(info.id) }).catch(() => {
      missing = true
    })
