...
// Loop-delegation tests: require the Rust smart-layer (DUO_SMART_LAYER_URL).
// Gated, not deleted (Rust has no equivalent loop coverage). See prompt.test.ts.
...
live("tool execution produces non-empty session diff (snapshot race)", () =>
  provideTmpdirServer(
