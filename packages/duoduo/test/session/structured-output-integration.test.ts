...
// Live LLM tests are opt-in. A stray ANTHROPIC_API_KEY in the developer shell
// must not make the regular unit suite depend on external provider config/network.
const runLiveLlmTests = process.env.DUODUO_RUN_LIVE_LLM_TESTS === "1" && !!process.env.ANTHROPIC_API_KEY
...
function run<A, E>(fx: Effect.Effect<A, E, unknown>) {
  const effect = fx.pipe(
    Effect.scoped,
    Effect.provide(Layer.mergeAll(SessionPrompt.defaultLayer, Session.defaultLayer, SessionCompletion.defaultLayer)),
  )
  return Effect.runPromise(effect as Effect.Effect<A, E, never>)
}

describe("StructuredOutput Integration", () => {
  test.skipIf(!runLiveLlmTests)(
