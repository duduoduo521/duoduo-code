...
describe("Discovery.pull", () => {
  const pull = (url: string) => {
    const effect = Discovery.Service.use((s) => s.pull(url)).pipe(Effect.provide(Discovery.defaultLayer))
    return Effect.runPromise(effect as Effect.Effect<string[], unknown, never>)
  }
