...
// Effect v4 beta: Effect.gen infers R as `unknown`.
// This helper casts it away so Effect.runPromise accepts it.
          ...
          await Instance.provide({
            ...
            fn: async () => {
              ...
              expect(extDirReq!.patterns).toContain(
                AppFileSystem.normalizePathPattern(path.join(process.env.WINDIR!, "*")),
              )
            },
          })
        }),
  ...
  if (process.platform === "win32") {
    test("normalizes external_directory workdir variants on Windows", async () => {
      ...
      await Instance.provide({
        ...
        fn: async () => {
          const bash = await initBash()
          const want = AppFileSystem.normalizePathPattern(path.join(outerTmp.path, "*"))
            ...
            fn: async () => {
              ...
              await expect(
                runPromise(
                  bash.execute(
                    {
                      command: "cat /tmp/duoduo-does-not-exist",
  ...
  each("always pattern has space before wildcard to not include different commands", async () => {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        const bash = await initBash()
        const requests: Array<Omit<Permission.Request, "id" | "sessionID" | "tool">> = []
        await runPromise(bash.execute({ command: "ls -la", description: "List" }, capture(requests)))
        const bashReq = requests.find((r) => r.permission === "bash")
        expect(bashReq).toBeDefined()
...
describe("tool.bash truncation", () => {
  test.skipIf(process.platform === "win32")("truncates output exceeding line limit", async () => {
    await Instance.provide({
      directory: projectRoot,
      fn: async () => {
        const bash = await initBash()
  ...
  test.skipIf(process.platform === "win32")("truncates output exceeding byte limit", async () => {
    await Instance.provide({
      directory: projectRoot,
      fn: async () => {
        const bash = await initBash()
  ...
  test.skipIf(process.platform === "win32")("full output is saved to file when truncated", async () => {
    await Instance.provide({
      directory: projectRoot,
      fn: async () => {
        const bash = await initBash()
