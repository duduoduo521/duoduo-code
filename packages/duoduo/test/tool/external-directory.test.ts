...
describe("tool.assertExternalDirectory", () => {
  ...
  if (process.platform === "win32") {
    test("normalizes Windows path variants to one glob", async () => {
      ...
      // Preserve the drive as a "/c/..." prefix so AppFileSystem.windowsPath maps it
      // back to the correct drive (C:). Dropping the drive letter (the old code) made
      // windowsPath re-derive "U:" from the leading "u", which never matches outerTmp.
      const alt = target
        .replace(/\\/g, "/")
        .replace(/^([A-Za-z]):/, "/$1")
        .toLowerCase()
