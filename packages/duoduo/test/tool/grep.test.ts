...
describe.skipIf(!hasRg)("tool.grep", () => {
  it.live("basic search", () =>
    provideTmpdirInstance((dir) =>
      Effect.gen(function* () {
        yield* Effect.promise(() => Bun.write(path.join(dir, "match.ts"), "export const a = 1\n"))
        yield* Effect.promise(() => Bun.write(path.join(dir, "other.txt"), "hello"))
        const info = yield* GrepTool
        const grep = yield* info.init()
        const result = yield* grep.execute({ pattern: "export", path: dir }, ctx)
