  ...
  if (process.platform === "win32") {
    it.live("normalizes read permission paths on Windows", () =>
      Effect.gen(function* () {
        ...
        // Keep the drive letter so path.isAbsolute recognizes it; only
        // normalize separators and casing to exercise the normalization logic.
...
describe("tool.read env file permissions", () => {
  // .env (exact) is hard-denied by Protected.isProtected before reaching the
  // permission ask() call. Variants like .env.local go through the permission
  // system and trigger "ask".
  const askCases: [string, boolean][] = [
    [".env.local", true],
    [".env.production", true],
  ...
  for (const agentName of ["build", "plan"] as const) {
    describe(`agent=${agentName}`, () => {
      it.live(".env is hard-denied (protected)", () =>
        Effect.gen(function* () {
          const dir = yield* tmpdirScoped()
          yield* put(path.join(dir, ".env"), "SECRET=1")

          const err = yield* provideInstance(dir)(
            Effect.gen(function* () {
              const exit = yield* run({ filePath: path.join(dir, ".env") }).pipe(Effect.exit)
              if (Exit.isFailure(exit)) {
                const e = Cause.squash(exit.cause)
                return e instanceof Error ? e : new Error(String(e))
              }
              throw new Error("expected .env read to be denied")
            }),
          )
      ...
      for (const [filename, shouldAsk] of askCases) {
