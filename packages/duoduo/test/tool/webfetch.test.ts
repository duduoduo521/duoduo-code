...
let activeFetch: (req: Request) => Response | Promise<Response> = (req) => globalThis.fetch(req)
...
async function withFetch(fetch: (req: Request) => Response | Promise<Response>, fn: (url: URL) => Promise<void>) {
  ...
  activeFetch = fetch
  try {
    // Use a public-looking hostname so the production SSRF guard remains active
    // in tests. The request never leaves the process because fetch is mocked.
    await fn(new URL("http://example.test/"))
  } finally {
...
function exec(args: { url: string; format: "text" | "markdown" | "html" }) {
  const mockFetch = ((input: RequestInfo | URL, init?: RequestInit) => {
    ...
    return Promise.resolve(activeFetch(req))
  }) as typeof fetch
