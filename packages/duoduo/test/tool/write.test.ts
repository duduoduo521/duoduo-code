...
describe("tool.write", () => {
  ...
  describe("error handling", () => {
    it.live("throws error when OS denies write access", () =>
      provideTmpdirInstance((dir) =>
        Effect.gen(function* () {
          const subdir = path.join(dir, "readonly-dir")
          yield* Effect.promise(() => fs.mkdir(subdir, { recursive: true }))
          const filePath = path.join(subdir, "readonly.txt")
          yield* Effect.promise(() => fs.writeFile(filePath, "test", "utf-8"))
          // Make both the file and its parent directory read-only.
          // The directory permission blocks rename() on platforms (macOS)
          // where file-level chmod 444 is bypassed by write-to-tmp-then-rename.
          yield* Effect.promise(() => fs.chmod(filePath, 0o444))
          yield* Effect.promise(() => fs.chmod(subdir, 0o555))
          const exit = yield* run({ filePath, content: "new content" }).pipe(Effect.exit)
          expect(exit._tag).toBe("Failure")
          // Restore write permission so cleanup can remove the directory
          yield* Effect.promise(() => fs.chmod(subdir, 0o755).catch(() => {}))
        }),
      ),
    )
  })
