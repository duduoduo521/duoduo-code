...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "duoduo-ai.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "duoduo-ai.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "duoduo-ai.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "duoduo-ai.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["duoduo-ai.json", "duoduo-ai.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "duoduo-ai.json"),
        path.join(tmp.path, "duoduo-ai.jsonc"),
        path.join(project, "duoduo-ai.json"),
        path.join(project, "duoduo-ai.jsonc"),
      ])
    })
  })
  ...
  describe("writeJson()", () => {
    ...
    test("writeJson uses atomic write (no partial file on crash)", async () => {
      await using tmp = await tmpdir()
      const filepath = path.join(tmp.path, "atomic.json")
      const data = { key: "atomic" }
    ...
    test("writeJson creates parent directories", async () => {
      await using tmp = await tmpdir()
      const filepath = path.join(tmp.path, "nested", "deep", "data.json")
  ...
  describe("resolve()", () => {
    ...
    test("resolves symlinked directory to canonical path", async () => {
      // Windows requires elevated privileges (Developer Mode / SeCreateSymbolicLinkPrivilege)
      // to create symlinks, so this cannot run reliably on Windows.
      if (process.platform === "win32") return
      await using tmp = await tmpdir()
    ...
    test("throws ELOOP on symlink cycle", async () => {
      // Windows requires elevated privileges to create symlinks; the cycle setup
      // itself throws EPERM before the resolve() ELOOP assertion can be exercised.
      if (process.platform === "win32") return
      await using tmp = await tmpdir()
