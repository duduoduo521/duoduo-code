...
const original = Flag.DUODUO_EXPERIMENTAL_WORKSPACES

beforeEach(() => {
  Database.close()
  Flag.DUODUO_EXPERIMENTAL_WORKSPACES = true
})

afterEach(async () => {
  mock.restore()
  await disposeAllWithTimeout()
  Flag.DUODUO_EXPERIMENTAL_WORKSPACES = original
  await resetDatabase()
})

// `Workspace.create` inserts a row whose `project_id` references the project
// table, so the project must be persisted first. Production does this in the
// `project.open` route (`fromDirectory(..., { create: true })`);
// `Instance.provide` alone only discovers the project, it does not persist it.
function open(dir: string) {
  return Effect.runPromise(
    Project.Service.use((svc) => svc.fromDirectory(dir, undefined, { create: true })).pipe(
      Effect.provide(Project.defaultLayer),
    ),
  )
}
