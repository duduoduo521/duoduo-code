/* oxlint-disable no-redundant-type-constituents -- ts7 type-aware preview resolves JSX.Element to an error/any type, misfiring this rule */
import type {
  AgentPart,
  DuoDuoClient,
  Event,
...
export type TuiDialogSelectOption<Value = unknown> = {
  ...
  value: Value
  description?: string
  // oxlint-disable-next-line no-redundant-type-constituents -- JSX.Element resolves to an error/any type under the ts7 preview; the string arm is intentional
...
export type TuiPluginInstallOptions = {
  global?: boolean
  /** E-01: allow plugins that declare `preinstall`/`install`/`postinstall` scripts. */
  allowScripts?: boolean
  /** E-01: expected SHA-256 of the resolved plugin package (fail-closed on mismatch). */
  integrity?: string
}
