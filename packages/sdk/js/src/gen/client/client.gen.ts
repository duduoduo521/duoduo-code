...
export const createClient = (config: Config = {}): Client => {
  ...
  const request: Client["request"] = async (options) => {
    ...
    // "fields" mode returns the error for the caller to inspect.
    // "data" mode has nowhere to put it, so throw instead of
    // silently returning `undefined` (the old behavior hid failures).
    if (opts.responseStyle === "data") {
      throw finalError
    }
