...
import { type DiffLineAnnotation } from "../diff-engine"
...
import { cloneLineSelection, formatSelectedLineLabel, lineInSelectedRange } from "../pierre/selection-bridge"
...
export type LineCommentAnnotationMeta<T> =
  ...
  | { kind: "draft"; key: string; range: LineSelection }
...
type LineCommentAnnotationsProps<T> = {
  ...
  getCommentSelection: (comment: T) => LineSelection
  draftRange: Accessor<LineSelection | null>
  draftKey: Accessor<string>
}

type LineCommentAnnotationsWithSideProps<T> = LineCommentAnnotationsProps<T> & {
  getSide: (range: LineSelection) => "additions" | "deletions"
}
...
type LineCommentStateProps<T> = {
  ...
  selected: Accessor<LineSelection | null>
  setSelected: (range: LineSelection | null) => void
  commenting: Accessor<LineSelection | null>
  setCommenting: (range: LineSelection | null) => void
  syncSelected?: (range: LineSelection | null) => void
  hoverSelected?: (range: LineSelection) => void
}
...
type LineCommentControllerProps<T extends LineCommentShape> = {
  ...
  onSubmit: (input: { comment: string; selection: LineSelection }) => void
  onUpdate?: (input: { id: string; comment: string; selection: LineSelection }) => void
  ...
  getHoverSelectedRange?: Accessor<LineSelection | null>
...
export function createLineCommentAnnotationRenderer<T>(props: {
  ...
  renderDraft: (range: LineSelection) => DraftProps
}) {
  const nodes = new Map<
    string,
    {
...
export function createLineCommentState<T>(props: LineCommentStateProps<T>) {
  ...
  const toRange = (range: LineSelection | null) => (range ? cloneLineSelection(range) : null)
  const setSelected = (range: LineSelection | null) => {
  ...
  const setCommenting = (range: LineSelection | null) => {
    const next = toRange(range)
  ...
  const openComment = (id: T, range: LineSelection, options?: { cancelDraft?: boolean }) => {
    if (options?.cancelDraft) cancelDraft()
    props.setOpened(id)
    setSelected(range)
  }

  const toggleComment = (id: T, range: LineSelection, options?: { cancelDraft?: boolean }) => {
    if (options?.cancelDraft) cancelDraft()
  ...
  const openDraft = (range: LineSelection) => {
    const next = toRange(range)
    setDraft("")
    setEditing(null)
    closeComment()
  ...
  const openEditor = (id: T, range: LineSelection, value: string) => {
    closeComment()
    setSelected(range)
    props.setCommenting(null)
    setEditing(id)
  ...
  const hoverComment = (range: LineSelection) => {
    const next = toRange(range)
    if (!next) return
  ...
  const finishSelection = (range: LineSelection) => {
    closeComment()
    setSelected(range)
    cancelDraft()
  }
...
export function createLineCommentController<T extends LineCommentShape>(
  ...
  onLineNumberSelectionEnd: (range: LineSelection | null) => void
}
export function createLineCommentController<T extends LineCommentShape>(
...
export function createLineCommentController<T extends LineCommentShape>(
  ...
  const { renderAnnotation } = createManagedLineCommentAnnotationRenderer<T>({
            ...
            ? {
                ...
                onSubmit: (value: string) => {
                  props.onUpdate?.({
                    id: comment.id,
                    comment: value,
                    selection: cloneLineSelection(comment.selection),
                  })
                  note.cancelDraft()
                },
    ...
    renderDraft: (range) => ({
      ...
      onCancel: note.cancelDraft,
      onSubmit: (comment) => {
        props.onSubmit({ comment, selection: cloneLineSelection(range) })
        note.cancelDraft()
      },
  ...
  const onLineSelected = (range: LineSelection | null) => {
    if (!range) {
      note.select(null)
      note.cancelDraft()
      return
  ...
  const onLineSelectionEnd = (range: LineSelection | null) => {
    if (!range) {
  ...
  const onLineNumberSelectionEnd = (range: LineSelection | null) => {
    if (!range) return
...
export function createLineCommentAnnotations<T>(
  ...
  const line = (range: LineSelection) => Math.max(range.start, range.end)
...
export function createLineCommentHoverRenderer(props: {
  label: string
  getSelectedRange: Accessor<LineSelection | null>
  onOpenDraft: (range: LineSelection) => void
}) {
    ...
    createHoverCommentUtility({
      ...
      onSelect: (hovered) => {
        ...
        if (current && lineInSelectedRange(current, hovered.lineNumber, hovered.side)) {
          props.onOpenDraft(cloneLineSelection(current))
          return
        }
        ...
        if (hovered.side) range.startSide = hovered.side
