...
// 子智能体类型 → 字面量 i18n key（避免动态拼接 key 触发类型错误；自定义 t 不支持 defaultValue）
const subagentTypeKey: Record<string, UiI18nKey> = {
  build: "ui.tool.agent.type.build",
  plan: "ui.tool.agent.type.plan",
  general: "ui.tool.agent.type.general",
  explore: "ui.tool.agent.type.explore",
  compaction: "ui.tool.agent.type.compaction",
  title: "ui.tool.agent.type.title",
  summary: "ui.tool.agent.type.summary",
  review: "ui.tool.agent.type.review",
}
...
import { getDirectory as _getDirectory, getFilename } from "@duoduo-ai/shared/util/path"
    ...
    case "task": {
      const rawType =
        typeof input.subagent_type === "string" && input.subagent_type
          ? input.subagent_type
          : ""
      const cap = rawType ? rawType[0]!.toUpperCase() + rawType.slice(1) : ""
      const key = (rawType && subagentTypeKey[rawType]) || "ui.tool.agent"
      return {
        icon: "task",
        title: agentTitle(i18n, rawType ? i18n.t(key, { type: cap }) : undefined),
...
function groupParts(parts: { messageID: string; part: PartType }[]) {
  ...
  for (let i = 0; i < parts.length; i++) {
    ...
    if (isContextGroupTool(item!.part)) {
      if (refs.length === 0) firstPartID = item!.part.id
      refs.push({ messageID: item!.messageID, partID: item!.part.id })
    } else {
      flushContext()
      result.push({
...
function renderable(part: PartType, showThinking = true) {
  if (part.type === "tool") {
  ...
  if (part.type === "reasoning") return !!showThinking && !!part.text?.trim()
  return !!PART_MAPPING[part.type]
}

// A tool part still stuck in pending/running while the turn is NOT working is a
// leftover from an interrupted run (e.g. the user clicked STOP): the delegated
// tool never got executed/completed and stays pending forever in the store.
// Such a part carries no useful output and renders as an empty "Shell" shimmer
// row, so it must be hidden once the turn is idle. While the turn IS working we
// keep showing it so live progress is visible.
function stalePendingTool(part: PartType, working: boolean) {
      ...
      groupParts(
        props.messages.flatMap((message) =>
          list(data.store.part?.[message.id], emptyParts)
            .filter((part) => renderable(part, props.showThinking ?? true))
            .filter((part) => !stalePendingTool(part, props.working ?? false))
            .map((part) => ({
...
function ContextToolGroup(props: { parts: ToolPart[]; busy?: boolean }) {
  ...
  const safeParts = createMemo(() => props.parts ?? [])
  const pending = createMemo(
    () =>
      !!props.busy || safeParts().some((part) => part.state.status === "pending" || part.state.status === "running"),
  )
  const summary = createMemo(() => contextToolSummary(safeParts()))
        ...
        <>
          ...
          <div data-slot="user-message-copy-wrapper">
            ...
            <Show when={props.actions?.retry}>
              <Tooltip value={i18n.t("ui.message.retryMessage")} placement="top" gutter={4}>
                <IconButton
                  icon="retry"
                  size="normal"
                  variant="ghost"
                  ...
                  aria-label={i18n.t("ui.message.retryMessage")}
                />
              </Tooltip>
            </Show>
          </div>
...
PART_MAPPING["text"] = function TextPartDisplay(props) {
  ...
  const meta = createMemo(() => {
    ...
    const tokenLabel = typeof total === "number" && total > 0 ? formatTokenCount(total) : ""
    const items = [
  ...
  return (
    <Show when={text()}>
      {/* data-streaming drives the breathing glow in message-part.css. It is
          set on the wrapper rather than the body so the glow can bleed past
          the text column without shifting any layout. */}
      <div data-component="text-part" data-streaming={streaming() ? "" : undefined}>
...
ToolRegistry.register({
  ...
  render(props) {
    ...
    const title = createMemo(() => {
      ...
      if (!name) return i18n.t("ui.tool.agent.default")
      // Localize the sub-agent type name (e.g. "explore" → "调查" in zh,
      // "Explore" in en) instead of showing the raw internal identifier.
      const key = subagentTypeKey[name]
      return agentTitle(i18n, key ? i18n.t(key) : name)
    })
                            ...
                            <Show when={visible()}>
                              <div data-component="apply-patch-file-diff">
                                <Dynamic
                                  component={fileComponent}
                                  mode="diff"
                                  before={{ contents: file.view.before }}
                                  after={{ contents: file.view.after }}
                                />
                              </div>
                            </Show>
                          </Accordion.Content>
            ...
            >
              <div data-component="apply-patch-file-diff">
                <Dynamic
                  component={fileComponent}
                  mode="diff"
                  before={{ contents: single()!.view.before }}
                  after={{ contents: single()!.view.after }}
                />
              </div>
            </ToolFileAccordion>
          </BasicTool>
