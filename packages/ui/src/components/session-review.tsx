...
const _ce = createEffect
function fxEffect(fn: () => unknown): void {
  const at = (new Error().stack ?? "").split("\n")[2]?.trim() ?? "?"
  _ce(() => {
    console.warn("[frontend] fx-run", at)
    return (fn as () => unknown)()
  })
}
import { createStore } from "solid-js/store"
import { type FileContent, type SnapshotFileDiff, type VcsFileDiff } from "@duoduo-ai/sdk/v2"
import { type LineSelection } from "../diff-engine/selection"
import { DiffViewer } from "../diff-engine/DiffViewer"
import { Dynamic } from "solid-js/web"
import { mediaKindFromPath } from "../pierre/media"
import { previewSelectedLines } from "../pierre/selection-bridge"
...
function langFromFile(path: string): string {
  const ext = path.lastIndexOf(".")
  if (ext === -1) return "text"
  const extension = path.slice(ext + 1).toLowerCase()
...
export const SessionReview = (props: SessionReviewProps) => {
  console.warn("[frontend] sr-start", props.diffs?.length)
  ...
  const items = createMemo<Item[]>(() => {
    console.warn("[frontend] items-start")
    const r = list(props.diffs).map((diff) => normalize(diff))
    console.warn("[frontend] items-end", r.length)
    return r
  })
  ...
  const selectionSide = (range: LineSelection) => range.endSide ?? range.startSide ?? "additions"

  const selectionPreview = (diff: ViewDiff, range: LineSelection) => {
  ...
  fxEffect(() => {
    ...
    untrack(() => {
      ...
      if (comment) setStore("selection", { file: comment.file, range: { ...comment.selection } })
  ...
  console.warn("[frontend] sr-render")
  return (
    <div data-component="session-review" class={props.class} classList={props.classList}>
      ...
      {(() => { console.warn("[frontend] sr-header-done"); return null })()}
          ...
          <Show when={hasDiffs()} fallback={props.empty}>
            <div class="pb-6">
              <Accordion multiple value={open()} onChange={handleChange}>
                <For each={items()}>
                  {(diff, i) => {
                    console.warn("[frontend] rw", i(), diff.file)
                    ...
                    const handleLineSelected = (range: LineSelection | null) => {
                      if (!props.onLineComment) return
                      commentsUi.onLineSelected(range)
                    }

                    const handleLineSelectionEnd = (range: LineSelection | null) => {
                      if (!props.onLineComment) return
                              ...
                              <Switch>
                                ...
                                <Match when={true}>
                                  <DiffViewer
                                    before={diff.before}
                                    after={diff.after}
                                    lang={langFromFile(diff.file)}
                                    diffStyle={diffStyle()}
                                    enableLineSelection={props.onLineComment != null}
                                    wordDiff={diffStyle() === "unified"}
