...
export function useFilteredList<T>(props: FilteredListProps<T>) {
  ...
  // Debounce filter changes to avoid per-keystroke fetch + fuzzysort.
  // When typing @file references, each keystroke triggers searchFilesAndDirectories
  // HTTP request + fuzzysort computation + SolidJS reactive cascade.
  // 150ms debounce batches rapid typing into a single search.
  ...
  const [grouped, { refetch }] = createResource(
    () => ({
      ...
      items: typeof props.items === "function" ? props.items(debouncedFilter()) : props.items,
    }),
    async ({ filter, items }) => {
      const query = filter ?? ""
  ...
  const onInput = (value: string) => {
    ...
    debounceTimer = setTimeout(() => {
      ...
      setDebouncedFilter(pendingFilter)
    }, DEBOUNCE_MS)
  }
