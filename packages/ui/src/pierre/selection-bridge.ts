import { type LineSelection, type DiffSide } from "../diff-engine/selection"
...
type Side = DiffSide | undefined
type LineSpan = Pick<LineSelection, "start" | "end">
...
export function cloneLineSelection(range: LineSelection): LineSelection {
  return { ...range }
}

export function lineInSelectedRange(range: LineSelection | null | undefined, line: number, side?: Side) {
...
export function isSingleLineSelection(range: LineSelection | null) {
  if (!range) return false
...
export function createLineNumberSelectionBridge() {
  ...
  return {
    ...
    consume(range: LineSelection | null) {
