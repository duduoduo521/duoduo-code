...
const THEME_STYLE_ID = "duoduo-theme"
...
function buildThemeCss(light: ResolvedTheme, dark: ResolvedTheme, themeId: string): string {
  // The zero theme's tokens are already baked into styles/theme.css, so it is
  // emitted at :root; every other theme is scoped to its own data-theme attr.
