...
import { blend, generateNeutralScale, generateScale, hexToOklch, hexToRgb, mixColors, shift, withAlpha } from "./color"
...
export function resolveThemeVariant(variant: ThemeVariant, isDark: boolean): ResolvedTheme {
  ...
  // Content panels sit on this, so in dark mode it must read as a distinct
  // step above `background-base` — neutral[1] left only ~2% luminance between
  // them, which made the whole layout look flat and undifferentiated.
  tokens["background-stronger"] = isDark ? neutral[3]! : "#fcfcfc"
  ...
  // Secondary / tertiary text: mix toward the background rather than applying a
  // fixed lightness offset. A fixed offset ignores how far `body` already sits
  // from the background, so a theme with a near-black ink produced a "weak"
  // colour almost as heavy as body text (~9:1 instead of the ~4.5:1 intended
  // for secondary copy), collapsing the typographic hierarchy.
  tokens["text-weak"] = colors.compact
    ? mixColors(body as HexColor, background, isDark ? 0.24 : 0.35)
    : neutral[8]!
  tokens["text-weaker"] = colors.compact
    ? mixColors(body as HexColor, background, isDark ? 0.42 : 0.55)
    : neutral[7]!
  ...
  // Alias of `border-weak-base`. Several components (the session-turn thinking
  // spinner ring, diff-engine dividers, file-preview table borders) reference
  // `border-subtle`, which was never emitted here — every usage resolved to an
  // invalid value at computed-value time and rendered nothing.
  tokens["border-subtle"] = tokens["border-weak-base"]!
  ...
  tokens["avatar-background-rose"] = isDark ? "#4d1426" : "#ffe9ef"
  tokens["avatar-background-amber"] = isDark ? "#4a3300" : "#fef3da"
  tokens["avatar-background-teal"] = isDark ? "#073a39" : "#e2f8f6"
  tokens["avatar-background-blue"] = isDark ? "#16294d" : "#e8f0fe"
  tokens["avatar-background-violet"] = isDark ? "#2c2150" : "#f0ecfd"
  tokens["avatar-background-emerald"] = isDark ? "#093d27" : "#e6f8ec"
  ...
  tokens["avatar-text-rose"] = isDark ? "#f485ac" : "#d6226b"
  tokens["avatar-text-amber"] = isDark ? "#f0b955" : "#b76e00"
  tokens["avatar-text-teal"] = isDark ? "#6fe0d7" : "#0f7d75"
  tokens["avatar-text-blue"] = isDark ? "#7aa6f0" : "#1c5fc4"
  tokens["avatar-text-violet"] = isDark ? "#b6a4f5" : "#5b3ec4"
  tokens["avatar-text-emerald"] = isDark ? "#6fe0a3" : "#128a4f"
...
function generateNeutralAlphaScale(neutralScale: HexColor[], isDark: boolean): ColorValue[] {
  // Semi-transparent overlay tones. A surface must composite correctly on top of
  // `background-base` *and* `background-stronger` — panels and cards commonly sit
  // on the lighter content area, so pre-blending the overlay against a single
  // base colour pins the result and yields visibly wrong panels on the other.
  const alphas = isDark
    ? [0.04, 0.07, 0.11, 0.16, 0.22, 0.29, 0.38, 0.5, 0.64, 0.78, 0.9, 0.98]
    : [0.03, 0.05, 0.085, 0.13, 0.18, 0.24, 0.32, 0.42, 0.55, 0.7, 0.85, 0.96]

  return alphas.map((alpha) => withAlpha(neutralScale[11]!, alpha) as ColorValue)
}
