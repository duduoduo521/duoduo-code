...
const model = "duoduo/gpt-5.3-codex"
...
async function build() {
  ...
  try {
    await $`./script/build.ts --single`.cwd("packages/duoduo")
    return true
  } catch (err) {
...
async function fix(pr: PR, files: string[], prs: PR[], applied: number[], idx: number) {
  console.log(`  Trying to auto-resolve ${files.length} conflict(s) with duoduocode...`)
  ...
  try {
    await $`duoduocode run -m ${model} ${prompt}`
  } catch (err) {
    console.log(`  duoduocode failed: ${err}`)
    return false
  }
  ...
  console.log("  Conflicts resolved with duoduocode")
  return true
}

async function smoke(prs: PR[], applied: number[]) {
  console.log("\nRunning final smoke check with duoduocode...")
  ...
  const prompt = [
    ...
    "Run `./script/build.ts --single` in `packages/duoduo`.",
