...
import { createDuoDuo } from "@duoduo-ai/sdk"
...
`)
  ...
  const duoduo = await createDuoDuo({ port: 0 })
  ...
  try {
    ...
    const session = await duoduo.client.session.create()
    const result = await duoduo.client.session
      .prompt({
